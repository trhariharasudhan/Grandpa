...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]Grandpa init[/bold] to detect hardware and generate "
        "[cyan]~/.Grandpa/config.toml[/cyan].\n"
        "  Or run [bold]Grandpa quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]Grandpa doctor[/bold] to check all engines.\n"
        "  Run [bold]Grandpa quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]Grandpa config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]Grandpa model list[/bold] to see available models."
        )
    return (
