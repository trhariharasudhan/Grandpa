...
def start(install: bool) -> None:
    ...
    if install:
        ...
        from grandpa.daemon.service import (
        ...
        if plat.system() == "Darwin":
            plist_path = (
                Path.home() / "Library/LaunchAgents/com.grandpa.gateway.plist"
            )
...
def stop() -> None:
    ...
    if plat.system() == "Darwin":
        subprocess.run(
            ["launchctl", "remove", "com.grandpa.gateway"],
            check=False,
        )
    else:
        subprocess.run(
...
def status() -> None:
    ...
    if plat.system() == "Darwin":
        subprocess.run(
            ["launchctl", "list", "com.grandpa.gateway"],
            check=False,
        )
    else:
        subprocess.run(
...
def logs() -> None:
    ...
    if plat.system() == "Darwin":
        click.echo("Check ~/Library/Logs/com.grandpa.gateway.log")
    else:
        subprocess.run(
