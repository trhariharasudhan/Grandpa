...
@click.option("--model", default=None, help="Offline Whisper/faster-whisper model name, e.g. tiny.en or base.en.")
@click.option("--language", default=None, help="Speech recognition language code, e.g. en. Empty means auto where supported.")
@click.option("--device", "stt_device", default=None, help="STT compute device: cpu, cuda, or auto.")
@click.option("--microphone", type=int, default=None, help="Microphone input device index.")
@click.option("--wake-word", is_flag=True, help="Wait for a wake phrase before listening for commands.")
@click.option(
    "--wake-phrase",
    multiple=True,
    help="Wake phrase to listen for. Can be passed more than once.",
)
@click.option("--no-wake-response", is_flag=True, help='Do not speak the "Yes?" wake acknowledgement.')
@click.option("--list-microphones", is_flag=True, help="List input microphone devices and exit.")
@click.option("--list-voices", is_flag=True, help="List local TTS voices and exit.")
@click.option("--diagnose", is_flag=True, help="Show voice dependencies and active Python environment, then exit.")
@click.pass_context
def voice(
    ctx: click.Context,
    no_tts: bool,
    ...
    try:
        ...
        click.echo(f"Voice mode could not initialize: {type(exc).__name__}: {exc}", err=True)
...
def doctor(device: int | None) -> None:
    _print_diagnostics(run_voice_doctor(device=device))
...
def _print_voices() -> None:
    ...
    if not voices:
        click.echo("No local TTS voices found. On Windows, pyttsx3 uses installed SAPI voices.")
        return
