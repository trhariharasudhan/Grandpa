...
class BrowserPageAnalyzer:
    ...
    def analyze(
        self, action: BrowserAwarenessAction, snapshot: BrowserPageSnapshot
    ) -> BrowserAwarenessResult:
        if not snapshot.supported:
            return BrowserAwarenessResult(
                "unsupported",
                snapshot.message or _unsupported_message(),
                action,
                snapshot,
            )
        ...
        if handler is None:
            return BrowserAwarenessResult(
                "unsupported",
                "That browser awareness action is not supported yet.",
                action,
                snapshot,
            )
    ...
    def _handle_title(
        ...
        return BrowserAwarenessResult(
            "handled", f"Title: {snapshot.title or 'Unknown'}", action, snapshot
        )
    ...
    def _handle_url(
        ...
        return BrowserAwarenessResult(
            "handled", f"URL: {snapshot.url or 'Unknown'}", action, snapshot
        )
    ...
    def _handle_read(
        ...
        if not snapshot.visible_text:
            return BrowserAwarenessResult(
                "unsupported",
                "Readable visible page text is not available yet.",
                action,
                snapshot,
            )
        return BrowserAwarenessResult(
            "handled", f"Visible content:\n{snapshot.visible_text}", action, snapshot
        )
    ...
    def _handle_find_text(
        ...
        if not query:
            return BrowserAwarenessResult(
                "unsupported",
                "Tell me what text to find on this page.",
                action,
                snapshot,
            )
        count = len(
            re.findall(re.escape(query), snapshot.visible_text, flags=re.IGNORECASE)
        )
        return BrowserAwarenessResult(
            "handled",
            f'Found "{query}" {count} time{"s" if count != 1 else ""} in visible content.',
            action,
            snapshot,
        )
    ...
    def _handle_selected_text(
        self, action: BrowserAwarenessAction, snapshot: BrowserPageSnapshot
    ) -> BrowserAwarenessResult:
        if not snapshot.selected_text:
            return BrowserAwarenessResult(
                "unsupported",
                "No selected browser text is available in the latest snapshot.",
                action,
                snapshot,
            )
        return BrowserAwarenessResult(
            "handled", f"Selected text:\n{snapshot.selected_text}", action, snapshot
        )
    ...
    def _handle_tabs(
        ...
        if not snapshot.tabs:
            return BrowserAwarenessResult(
                "unsupported",
                "Browser tab titles are not safely available yet.",
                action,
                snapshot,
            )
        ...
        lines.extend(
            f"{index}. {title}"
            for index, title in enumerate(snapshot.tabs[:12], start=1)
        )
        return BrowserAwarenessResult("handled", "\n".join(lines), action, snapshot)
