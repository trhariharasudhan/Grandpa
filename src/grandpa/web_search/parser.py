        ...
        for pattern, mode in patterns:
            ...
            if mode == "news_days":
                ...
                return WebSearchAction(
                    "search", WebSearchQuery(query, mode="news", recency_days=days)
                )
            if mode == "week":
                query = raw[match.start(1) : match.end(1)]
                return WebSearchAction(
                    "search", WebSearchQuery(query, mode="news", recency_days=7)
                )
            if mode == "top":
                count = int(match.group(1))
                query = raw[match.start(2) : match.end(2)]
                return WebSearchAction(
                    "search", WebSearchQuery(query, max_results=min(count, 10))
                )
            if mode == "official_docs":
                ...
                return WebSearchAction(
                    "search",
                    WebSearchQuery(
                        f"official {domain_hint} docs {query}", official_only=True
                    ),
                )
            query = raw[match.start(1) : match.end(1)]
            if mode == "official":
                return WebSearchAction(
                    "search", WebSearchQuery(query, official_only=True)
                )
            if mode == "news":
                return WebSearchAction(
                    "search", WebSearchQuery(query, mode="news", recency_days=1)
                )
