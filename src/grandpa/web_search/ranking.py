...
class WebSearchRanker:
    ...
    def rank(
        self, results: tuple[WebSearchResult, ...], query: WebSearchQuery
        ...
        for index, result in enumerate(results):
            ...
            if query.official_only or any(
                marker in domain for marker in OFFICIAL_MARKERS
            ):
                score += 20
            if query.recency_days is not None and result.published_at:
                score += 8
            if any(
                marker in domain for marker in SPAM_MARKERS
            ) or self.safety.is_low_quality(result.url):
                score -= 40
