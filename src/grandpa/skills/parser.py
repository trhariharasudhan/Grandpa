...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.grandpa.* via FIELD_MAPPING.
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.grandpa.original_frontmatter,
        and merges metadata.grandpa.* into canonical fields.
        """
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.grandpa.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.grandpa.* into canonical fields (these override
        ...
        # Preserve unmapped fields under metadata.grandpa.original_frontmatter
        if unmapped:
