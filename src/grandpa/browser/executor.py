...
class BrowserExecutor:
    ...
    def __init__(self, opener: OpenCallback | None = None, hotkey_runner: HotkeyCallback | None = None) -> None:
    ...
    def execute(self, action: BrowserAction) -> BrowserOperationResult:
        if action.action == "open_url":
            return self._open_url(action, action.url or action.target, label=action.target or "Website")
        ...
        return BrowserOperationResult("unsupported", "That browser action is not supported yet.", action)

    def _open_url(self, action: BrowserAction, url: str, *, label: str) -> BrowserOperationResult:
        ...
        try:
            ...
            return BrowserOperationResult("error", f"Could not open {label}.", action, normalized, str(exc))
        if not opened:
            return BrowserOperationResult("error", f"Could not open {label}.", action, normalized)
    ...
    def _search(self, action: BrowserAction) -> BrowserOperationResult:
        ...
        if result is None:
            return BrowserOperationResult("unsupported", "That search provider is not supported yet.", action)
        label, url = result
        opened = self._open_url(action, url, label=label)
        if opened.status != "handled":
            return opened
        return BrowserOperationResult("handled", f"Searching {label} for {action.query}.", action, opened.url)

    def _open_page(self, action: BrowserAction) -> BrowserOperationResult:
        url = PAGE_URLS.get(action.target)
        if url is None:
            return BrowserOperationResult("unsupported", "That browser page is not supported yet.", action)
        try:
            opened = self.opener(url)
        except Exception as exc:
            return BrowserOperationResult("error", f"Could not open browser {action.target}.", action, url, str(exc))
        if not opened:
            return BrowserOperationResult("error", f"Could not open browser {action.target}.", action, url)
        return BrowserOperationResult("handled", f"Browser {action.target} opened.", action, url)
    ...
    def _hotkey(self, action: BrowserAction) -> BrowserOperationResult:
        ...
        try:
            ...
            return BrowserOperationResult("error", "Could not send that browser shortcut.", action, error=str(exc))
        if not ok:
            return BrowserOperationResult("error", "Could not send that browser shortcut.", action)
        messages = {
...
def _default_hotkey(keys: tuple[str, ...]) -> bool:
    ...
    response = run_local_action({"action_type": "keyboard_hotkey", "target": "+".join(keys), "args": {"keys": list(keys)}})
