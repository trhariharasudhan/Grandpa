...
registered as a standard grandpa agent so the rest of the platform
...
Items in parentheses are what the *paper* trains. These grandpa ports
...
`src/grandpa/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).

## Quickstart

```bash
cd /matx/u/aspark/grandpa
source .env                                           # API keys
...
# 3. Run a smoke cell
.venv/bin/python -m grandpa.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$grandpa_HYBRID_EXPERIMENTS_DIR/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.grandpa-hybrid/experiments/`). The schema matches the
...
```bash
src/grandpa/agents/hybrid/scripts/new_experiment.sh \
...
these grandpa cells should reproduce the harness numbers within
