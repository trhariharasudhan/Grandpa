...
class FileAssistantStore:
    ...
    def recent(
        self, limit: int = 20, since: float | None = None
    ) -> list[dict[str, Any]]:
        clauses: list[str] = []
        params: list[Any] = []
...
def handle_file_command(
    ...
    if not automation.should_fallback:
        if automation.status == "handled" and automation.path is not None:
            store.record(
                automation.action.action if automation.action else "file",
                automation.path,
                automation.status,
            )
    ...
    match = re.fullmatch(
        r"find (?:excel|xlsx|spreadsheet|word|docx|powerpoint|pptx|resume|invoice)(?: files| sheets| documents)?(?: about (.+))?",
        command,
    )
    if match:
    ...
    if command == "open my vs code project":
        ...
        return FileAssistantResult(
            ...
            "file",
            str(workspace),
            "I could not find the Grandpa workspace.",
        )
...
def _document_search(query: str, store: FileAssistantStore) -> FileAssistantResult:
    ...
    return FileAssistantResult(
        result.status, "file", query, result.message, "I searched your local documents."
    )
...
def _document_diagnostics() -> FileAssistantResult:
    ...
    counts = (
        ", ".join(
            f"{key}: {value}" for key, value in sorted(data["type_counts"].items())
        )
        or "none indexed yet"
    )
    message = (
    ...
    return FileAssistantResult(
        "handled", "file", "diagnostics", message, "File diagnostics are ready."
    )
...
def _organization_dry_run(query: str) -> FileAssistantResult:
    ...
    return FileAssistantResult(
        ...
        "organization_plan",
        result.message,
        "Prepared an organization dry run.",
    )
...
def _show_recent_files(store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not rows:
        return FileAssistantResult(
            ...
            "file",
            "recent",
            "I do not have recent file activity yet.",
            "No recent files yet.",
        )
    lines = ["Recent files:"]
    for row in rows[:10]:
...
def _files_used_today(store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not rows:
        return FileAssistantResult(
            ...
            "file",
            "today",
            "I do not have file activity recorded for today yet.",
        )
...
def _find_files(extension: str, store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not matches:
        return FileAssistantResult(
            ...
            "file",
            ext,
            f"I did not find any {extension.upper()} files in the safe search folders.",
        )
    lines = [f"Found {len(matches)} {extension.upper()} file(s):"]
    for path in matches[:10]:
        meta = _metadata(path)
        lines.append(
            f"- {path.name} — {meta['size_label']}, modified {meta['modified_label']}"
        )
...
def _search_files(query: str, store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not matches:
        return FileAssistantResult(
            ...
            "file",
            query,
            f"I did not find files about {query} in the safe search folders.",
        )
...
def _create_note(raw_name: str, store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not name:
        return FileAssistantResult(
            ...
            "file",
            raw_name,
            "I blocked this note name for safety.",
            permission="blocked",
        )
    NOTES_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if path.exists():
        ...
        return FileAssistantResult(
            "handled", "file", str(path), f"The note already exists: {path.name}"
        )
    ...
    return FileAssistantResult(
        "handled", "file", str(path), f"Created note: {path.name}", "Created the note."
    )
...
def _read_latest_note(store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not notes:
        return FileAssistantResult(
            "handled", "file", "notes", "I do not have any notes yet."
        )
...
def _read_named_document(name: str, store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not path:
        return FileAssistantResult(
            ...
            "file",
            name,
            f"I could not find a readable file matching {name}.",
        )
...
def _summarize_latest_pdf(store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not pdfs:
        return FileAssistantResult(
            ...
            "file",
            "pdf",
            "I did not find a PDF to summarize in the safe search folders.",
        )
    return _summarize_document(pdfs[0], store)


def _summarize_named_document(
    name: str, store: FileAssistantStore
) -> FileAssistantResult:
    path = _find_best_match(name)
    if not path:
        return FileAssistantResult(
            "handled", "file", name, f"I could not find a document matching {name}."
        )
...
def _read_document(path: Path, store: FileAssistantStore) -> FileAssistantResult:
    if path.suffix.lower() not in DOCUMENT_EXTENSIONS:
        return FileAssistantResult(
            ...
            "file",
            str(path),
            "I can read txt, md, and PDF files in this phase.",
        )
    text = _extract_text(path)
    if not text:
        return FileAssistantResult(
            "unsupported", "file", str(path), f"I could not read text from {path.name}."
        )
...
def _summarize_document(path: Path, store: FileAssistantStore) -> FileAssistantResult:
    ...
    if not text:
        return FileAssistantResult(
            ...
            "file",
            str(path),
            f"I could not extract text from {path.name}.",
        )
...
def _open_path(path: Path, store: FileAssistantStore) -> FileAssistantResult:
    if not path.exists():
        return FileAssistantResult(
            "unsupported", "file", str(path), f"I could not find {path}."
        )
    if sys.platform != "win32":
        return FileAssistantResult(
            ...
            "file",
            str(path),
            "Opening files and folders is only supported on Windows desktop here.",
        )
    try:
        os.startfile(path)  # type: ignore[attr-defined]  # noqa: S606
    except OSError as exc:
        return FileAssistantResult(
            "error", "file", str(path), f"I could not open {path}: {exc}"
        )
    store.record("open", path, "opened path")
    _record_memory_activity("file", "open", str(path), path.name, "handled")
    return FileAssistantResult(
        "handled", "file", str(path), f"Opening {path.name}.", f"Opening {path.name}."
    )
...
def _find_best_match(query: str) -> Path | None:
    ...
    candidates = (
        _iter_safe_files() + list(NOTES_DIR.glob("*.md"))
        if NOTES_DIR.exists()
        else _iter_safe_files()
    )
...
def _record_memory_activity(
    category: str, action: str, target: str, detail: str, status: str
) -> None:
    try:
