...
class FileExecutor:
    ...
    def execute(self, action: FileAction, *, confirm: ConfirmationCallback | None = None) -> FileOperationResult:
        try:
            ...
            return FileOperationResult("error", f"I could not complete that file action: {exc}", action, error=exc.__class__.__name__)
        return FileOperationResult("unsupported", "This file action is not supported yet.", action)
    ...
    def _create(self, action: FileAction, *, folder: bool) -> FileOperationResult:
        ...
        if path.exists() and not action.overwrite:
            return FileOperationResult("needs_confirmation", "Destination already exists. Confirm overwrite.", action, path, requires_confirmation=True)
        if folder:
            path.mkdir(parents=True, exist_ok=action.overwrite)
            return FileOperationResult("handled", f"Folder created: {describe_path(path)}", action, path)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("", encoding="utf-8")
        return FileOperationResult("handled", f"File created: {describe_path(path)}", action, path)
    ...
    def _rename(self, action: FileAction) -> FileOperationResult:
        ...
        return self._move_or_copy(action, source, destination, move=True, label="renamed")
    ...
    def _copy(self, action: FileAction) -> FileOperationResult:
        ...
        return self._move_or_copy(action, source, destination, move=False, label="copied")
    ...
    def _delete(self, action: FileAction, *, confirm: ConfirmationCallback | None) -> FileOperationResult:
        source = self._resolve_existing(action.source)
        if isinstance(source, FileOperationResult):
            return source
        ...
        if self.safety.blocks_recursive_delete(source):
            return FileOperationResult("blocked", "Recursive deletion of that user folder is blocked.", action, source)
        ...
        if source.is_dir():
            ...
            source.unlink()
        return FileOperationResult("handled", f"Deleted: {describe_path(source)}", action, source)

    def _search(self, action: FileAction) -> FileOperationResult:
        if action.args.get("latest"):
            path = latest_by_suffix(set(action.args.get("suffixes") or ()), roots=self.roots)
            if path is None:
                return FileOperationResult("handled", "No matching files found.", action)
            return FileOperationResult("handled", f"Latest match: {describe_path(path)}", action, path, matches=(path,))
        matches = tuple(find_matches(action.query, roots=self.roots))
        suffixes = {str(suffix).casefold() for suffix in action.args.get("suffixes", ())}
        if suffixes:
            matches = tuple(path for path in matches if path.suffix.casefold() in suffixes)
        if not matches:
            return FileOperationResult("handled", "No matching files found.", action)
        lines = [f"Found {len(matches)} matching file(s):"]
        lines.extend(f"{index}. {path}" for index, path in enumerate(matches[:10], start=1))
    ...
    def _open(self, action: FileAction) -> FileOperationResult:
        ...
        if isinstance(source, FileOperationResult):
            return source
        self.opener(source)
        return FileOperationResult("handled", f"Opening {describe_path(source)}.", action, source)
    ...
    def _open_containing_folder(self, action: FileAction) -> FileOperationResult:
        ...
        self.opener(folder)
        return FileOperationResult("handled", f"Opening containing folder: {describe_path(folder)}.", action, folder)
    ...
    def _zip(self, action: FileAction) -> FileOperationResult:
        ...
        if destination.exists() and not action.overwrite:
            return FileOperationResult("needs_confirmation", "Destination archive already exists. Confirm overwrite.", action, source, destination, requires_confirmation=True)
        ...
        return FileOperationResult("handled", f"Archive created: {describe_path(destination)}", action, source, destination)
    ...
    def _extract(self, action: FileAction) -> FileOperationResult:
        ...
        if source.suffix.casefold() != ".zip":
            return FileOperationResult("unsupported", "This archive format is not supported yet.", action, source)
        destination = resolve_destination(action.destination, roots=self.roots) if action.destination else source.with_suffix("")
        blocked = self._blocked_path(destination, action)
        if blocked:
            return blocked
        ...
        with zipfile.ZipFile(source, "r") as archive:
            for member in archive.infolist():
                ...
                if member_path.is_absolute() or ".." in member_path.parts:
                    return FileOperationResult("blocked", "Archive contains unsafe paths.", action, source, destination)
                target = (destination / member.filename).resolve(strict=False)
                if not str(target).casefold().startswith(str(destination.resolve(strict=False)).casefold()):
                    return FileOperationResult("blocked", "Archive contains unsafe paths.", action, source, destination)
            archive.extractall(destination)
        return FileOperationResult("handled", f"Archive extracted to {describe_path(destination)}", action, source, destination)
    ...
    def _properties(self, action: FileAction) -> FileOperationResult:
        ...
        if isinstance(source, FileOperationResult):
            return source
        stat = source.stat()
        kind = "folder" if source.is_dir() else (source.suffix.lower().lstrip(".") or "file")
        message = (
    ...
    def _move_or_copy(self, action: FileAction, source: Path, destination: Path, *, move: bool, label: str) -> FileOperationResult:
        blocked = self._blocked_path(destination, action) or self._blocked_path(source, action)
        if blocked:
            return blocked
        if destination.exists() and not action.overwrite:
            return FileOperationResult("needs_confirmation", "Destination already exists. Confirm overwrite.", action, source, destination, requires_confirmation=True)
        ...
        return FileOperationResult("handled", f"File {label} to {describe_path(destination)}.", action, source, destination)
    ...
    def _destination_for(self, action: FileAction, source: Path) -> Path:
        if action.destination:
            ...
            if action.destination.casefold() in {"desktop", "documents", "downloads", "pictures", "music", "videos", "home", "project", "grandpa project"}:
                return destination / source.name
            return destination
    ...
    def _resolve_existing(self, value: str) -> Path | FileOperationResult:
        ...
        if len(matches) > 1:
            ...
            lines.extend(f"{index}. {match}" for index, match in enumerate(matches[:10], start=1))
            return FileOperationResult("ambiguous", "\n".join(lines), matches=tuple(matches))
        return FileOperationResult("error", f"I could not find {raw}.", error="missing_path")

    def _blocked_path(self, path: Path, action: FileAction) -> FileOperationResult | None:
        if self.safety.blocks_traversal(action.source) or (action.destination and self.safety.blocks_traversal(action.destination)):
            return FileOperationResult("blocked", "Path traversal is blocked.", action, path)
        if self.safety.is_protected(path):
            return FileOperationResult("blocked", "That path is protected and cannot be modified.", action, path)
        return None
