...
class FileSafetyPolicy:
    ...
    def is_protected(self, path: Path) -> bool:
        ...
        if len(parts) >= 2 and parts[0].endswith(":") and parts[1] in PROTECTED_WINDOWS_ROOT_NAMES:
            return True
    ...
    def blocks_traversal(self, raw: str) -> bool:
        return any(part == ".." for part in PureWindowsPath(str(raw).replace("/", "\\")).parts)

    def delete_requires_confirmation(self, path: Path) -> bool:
        return True

    def blocks_recursive_delete(self, path: Path) -> bool:
        protected_names = {"desktop", "documents", "downloads", "pictures", "music", "videos"}
...
def _normalised_parts(path: Path) -> tuple[str, ...]:
    ...
    if len(raw) >= 2 and raw[1] == ":":
        return tuple(part.rstrip("\\/").casefold() for part in PureWindowsPath(raw).parts if part.rstrip("\\/"))
    return tuple(part.casefold() for part in path.expanduser().resolve(strict=False).parts if part)
