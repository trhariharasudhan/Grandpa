...
from grandpa.evals.core.display import (
    print_banner,
    print_completion,
    print_full_results,
    print_run_header,
...
def _build_backend(
    ...
    if backend_name == "Grandpa-agent":
        from grandpa.evals.backends.grandpa_agent import GrandpaAgentBackend
        ...
        from grandpa.evals.backends.grandpa_direct import GrandpaDirectBackend
        ...
        from grandpa.evals.backends.external import OpenClawBackend
...
def _build_dataset(benchmark: str, subset: str | None = None):
    ...
    if benchmark == "supergpqa":
        from grandpa.evals.datasets.supergpqa import SuperGPQADataset

        return SuperGPQADataset()
    elif benchmark == "gpqa":
        from grandpa.evals.datasets.gpqa import GPQADataset

        return GPQADataset()
    elif benchmark == "mmlu-pro":
        from grandpa.evals.datasets.mmlu_pro import MMLUProDataset

        return MMLUProDataset()
    elif benchmark == "math500":
        from grandpa.evals.datasets.math500 import MATH500Dataset

        return MATH500Dataset()
    elif benchmark == "natural-reasoning":
        from grandpa.evals.datasets.natural_reasoning import NaturalReasoningDataset

        return NaturalReasoningDataset()
    elif benchmark == "hle":
        from grandpa.evals.datasets.hle import HLEDataset

        return HLEDataset()
    elif benchmark == "simpleqa":
        from grandpa.evals.datasets.simpleqa import SimpleQADataset

        return SimpleQADataset()
    elif benchmark == "wildchat":
        from grandpa.evals.datasets.wildchat import WildChatDataset

        return WildChatDataset()
    elif benchmark == "ipw":
        from grandpa.evals.datasets.ipw_mixed import IPWDataset
        ...
        from grandpa.evals.datasets.frames import FRAMESDataset
        ...
        from grandpa.evals.datasets.swefficiency import SWEfficiencyDataset

        return SWEfficiencyDataset()
    elif benchmark == "terminalbench":
        from grandpa.evals.datasets.terminalbench import TerminalBenchDataset

        return TerminalBenchDataset()
    elif benchmark == "terminalbench-native":
        from grandpa.evals.datasets.terminalbench_native import (
            TerminalBenchNativeDataset,
        )

        return TerminalBenchNativeDataset()
    elif benchmark == "terminalbench-v2.1":
        from grandpa.evals.datasets.terminalbench_v2_1 import (
            TerminalBenchV21Dataset,
        )

        return TerminalBenchV21Dataset()
    elif benchmark == "email_triage":
        from grandpa.evals.datasets.email_triage import EmailTriageDataset

        return EmailTriageDataset()
    elif benchmark == "morning_brief":
        from grandpa.evals.datasets.morning_brief import MorningBriefDataset

        return MorningBriefDataset()
    elif benchmark == "research_mining":
        from grandpa.evals.datasets.research_mining import ResearchMiningDataset

        return ResearchMiningDataset()
    elif benchmark == "knowledge_base":
        from grandpa.evals.datasets.knowledge_base import KnowledgeBaseDataset

        return KnowledgeBaseDataset()
    elif benchmark == "coding_task":
        from grandpa.evals.datasets.coding_task import CodingTaskDataset

        return CodingTaskDataset()
    elif benchmark == "loghub":
        from grandpa.evals.datasets.loghub import LogHubDataset

        return LogHubDataset()
    elif benchmark == "ama-bench":
        from grandpa.evals.datasets.ama_bench import AMABenchDataset

        return AMABenchDataset()
    elif benchmark == "lifelong-agent":
        from grandpa.evals.datasets.lifelong_agent import LifelongAgentDataset

        return LifelongAgentDataset(subset=subset or "db_bench")
    elif benchmark == "deepplanning":
        from grandpa.evals.datasets.deepplanning import DeepPlanningDataset

        return DeepPlanningDataset()
    elif benchmark == "paperarena":
        from grandpa.evals.datasets.paperarena import PaperArenaDataset

        return PaperArenaDataset()
    elif benchmark == "webchorearena":
        from grandpa.evals.datasets.webchorearena import WebChoreArenaDataset

        return WebChoreArenaDataset()
    elif benchmark == "workarena":
        from grandpa.evals.datasets.workarena import WorkArenaDataset

        return WorkArenaDataset()
    elif benchmark == "coding_assistant":
        from grandpa.evals.datasets.coding_assistant import CodingAssistantDataset
        ...
        from grandpa.evals.datasets.daily_digest import DailyDigestDataset

        return DailyDigestDataset()
    elif benchmark == "doc_qa":
        from grandpa.evals.datasets.doc_qa import DocQADataset

        return DocQADataset()
    elif benchmark == "browser_assistant":
        from grandpa.evals.datasets.browser_assistant import BrowserAssistantDataset
        ...
        from grandpa.evals.datasets.taubench import TauBenchDataset

        domains = subset.split(",") if subset else None
        return TauBenchDataset(domains=domains)
    elif benchmark == "livecodebench":
        from grandpa.evals.datasets.livecodebench import LiveCodeBenchDataset

        return LiveCodeBenchDataset()
    elif benchmark in ("liveresearch", "deepresearch"):
        from grandpa.evals.datasets.liveresearch import LiveResearchBenchDataset

        return LiveResearchBenchDataset(path=subset)
    elif benchmark == "liveresearchbench":
        from grandpa.evals.datasets.liveresearchbench import (
...
def _build_scorer(benchmark: str, judge_backend, judge_model: str):
    ...
    if benchmark == "supergpqa":
        from grandpa.evals.scorers.supergpqa_mcq import SuperGPQAScorer

        return SuperGPQAScorer(judge_backend, judge_model)
    elif benchmark == "gpqa":
        from grandpa.evals.scorers.gpqa_mcq import GPQAScorer

        return GPQAScorer(judge_backend, judge_model)
    elif benchmark == "mmlu-pro":
        from grandpa.evals.scorers.mmlu_pro_mcq import MMLUProScorer

        return MMLUProScorer(judge_backend, judge_model)
    elif benchmark == "math500" or benchmark == "natural-reasoning":
        from grandpa.evals.scorers.reasoning_judge import ReasoningJudgeScorer

        return ReasoningJudgeScorer(judge_backend, judge_model)
    elif benchmark == "hle":
        from grandpa.evals.scorers.hle_judge import HLEScorer

        return HLEScorer(judge_backend, judge_model)
    elif benchmark == "simpleqa":
        from grandpa.evals.scorers.simpleqa_judge import SimpleQAScorer

        return SimpleQAScorer(judge_backend, judge_model)
    elif benchmark == "wildchat":
        from grandpa.evals.scorers.wildchat_judge import WildChatScorer

        return WildChatScorer(judge_backend, judge_model)
    elif benchmark == "ipw":
        from grandpa.evals.scorers.ipw_mixed import IPWMixedScorer

        return IPWMixedScorer(judge_backend, judge_model)
    elif benchmark == "gaia":
        from grandpa.evals.scorers.gaia_exact import GAIAScorer

        return GAIAScorer(judge_backend, judge_model)
    elif benchmark == "frames":
        from grandpa.evals.scorers.frames_judge import FRAMESScorer

        return FRAMESScorer(judge_backend, judge_model)
    elif benchmark == "swebench":
        from grandpa.evals.scorers.swebench_structural import SWEBenchScorer

        return SWEBenchScorer(judge_backend, judge_model)
    elif benchmark == "swefficiency":
        from grandpa.evals.scorers.swefficiency_structural import SWEfficiencyScorer

        return SWEfficiencyScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench":
        from grandpa.evals.scorers.terminalbench_judge import TerminalBenchScorer

        return TerminalBenchScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench-native":
        from grandpa.evals.scorers.terminalbench_native_structural import (
            TerminalBenchNativeScorer,
        )

        return TerminalBenchNativeScorer(judge_backend, judge_model)
    elif benchmark == "terminalbench-v2.1":
        from grandpa.evals.scorers.terminalbench_v2_1 import (
            TerminalBenchV21Scorer,
        )

        return TerminalBenchV21Scorer(judge_backend, judge_model)
    elif benchmark == "email_triage":
        from grandpa.evals.scorers.email_triage import EmailTriageScorer

        return EmailTriageScorer(judge_backend, judge_model)
    elif benchmark == "morning_brief":
        from grandpa.evals.scorers.morning_brief import MorningBriefScorer

        return MorningBriefScorer(judge_backend, judge_model)
    elif benchmark == "research_mining":
        from grandpa.evals.scorers.research_mining import ResearchMiningScorer

        return ResearchMiningScorer(judge_backend, judge_model)
    elif benchmark == "knowledge_base":
        from grandpa.evals.scorers.knowledge_base import KnowledgeBaseScorer

        return KnowledgeBaseScorer(judge_backend, judge_model)
    elif benchmark == "coding_task":
        from grandpa.evals.scorers.coding_task import CodingTaskScorer

        return CodingTaskScorer(judge_backend, judge_model)
    elif benchmark == "loghub":
        from grandpa.evals.scorers.loghub_scorer import LogHubScorer

        return LogHubScorer(judge_backend, judge_model)
    elif benchmark == "ama-bench":
        from grandpa.evals.scorers.ama_bench_judge import AMABenchScorer

        return AMABenchScorer(judge_backend, judge_model)
    elif benchmark == "lifelong-agent":
        from grandpa.evals.scorers.lifelong_agent_scorer import LifelongAgentScorer

        return LifelongAgentScorer(judge_backend, judge_model)
    elif benchmark == "deepplanning":
        from grandpa.evals.scorers.deepplanning_scorer import DeepPlanningScorer

        return DeepPlanningScorer(judge_backend, judge_model)
    elif benchmark == "paperarena":
        from grandpa.evals.scorers.paperarena_judge import PaperArenaScorer

        return PaperArenaScorer(judge_backend, judge_model)
    elif benchmark == "webchorearena":
        from grandpa.evals.scorers.webchorearena_scorer import WebChoreArenaScorer

        return WebChoreArenaScorer(judge_backend, judge_model)
    elif benchmark == "workarena":
        from grandpa.evals.scorers.workarena_scorer import WorkArenaScorer

        return WorkArenaScorer(judge_backend, judge_model)
    elif benchmark == "coding_assistant":
        from grandpa.evals.scorers.coding_assistant import CodingAssistantScorer

        return CodingAssistantScorer(judge_backend, judge_model)
    elif benchmark == "security_scanner":
        from grandpa.evals.scorers.security_scanner import SecurityScannerScorer

        return SecurityScannerScorer(judge_backend, judge_model)
    elif benchmark == "daily_digest":
        from grandpa.evals.scorers.daily_digest import DailyDigestScorer

        return DailyDigestScorer(judge_backend, judge_model)
    elif benchmark == "doc_qa":
        from grandpa.evals.scorers.doc_qa import DocQAScorer

        return DocQAScorer(judge_backend, judge_model)
    elif benchmark == "browser_assistant":
        from grandpa.evals.scorers.browser_assistant import BrowserAssistantScorer

        return BrowserAssistantScorer(judge_backend, judge_model)
    elif benchmark == "pinchbench":
        from grandpa.evals.scorers.pinchbench import PinchBenchScorer

        return PinchBenchScorer(judge_backend, judge_model)
    elif benchmark == "taubench":
        from grandpa.evals.scorers.taubench import TauBenchScorer

        return TauBenchScorer(judge_backend, judge_model)
    elif benchmark == "livecodebench":
        from grandpa.evals.scorers.livecodebench import LiveCodeBenchScorer

        return LiveCodeBenchScorer(judge_backend, judge_model)
    elif benchmark in ("liveresearch", "deepresearch"):
        from grandpa.evals.scorers.liveresearch import LiveResearchBenchScorer

        return LiveResearchBenchScorer(judge_backend, judge_model)
    elif benchmark == "liveresearchbench":
        from grandpa.evals.scorers.liveresearchbench import (
            LiveResearchBenchScorer as LRBScorer,
        )

        return LRBScorer(judge_backend, judge_model)
    elif benchmark == "toolcall15":
        from grandpa.evals.scorers.toolcall15 import ToolCall15Scorer
...
def _build_trackers(config) -> list:
    ...
    if getattr(config, "wandb_project", ""):
        try:
            from grandpa.evals.trackers.wandb_tracker import WandbTracker
    ...
    if getattr(config, "sheets_spreadsheet_id", ""):
        try:
            from grandpa.evals.trackers.sheets_tracker import SheetsTracker
...
def _run_terminalbench_native(config, console: Console) -> object:
    ...
    from grandpa.evals.backends.terminalbench_native import (
        TerminalBenchNativeBackend,
    )
    from grandpa.evals.core.types import RunSummary
...
def _run_single(config, console: Optional[Console] = None) -> object:
    ...
    from grandpa.evals.core.runner import EvalRunner
...
def _run_agentic(
    ...
    *,
    ...
    from grandpa.evals.core.agentic_runner import AgenticRunner
    from grandpa.evals.core.event_recorder import EventRecorder
    from grandpa.evals.core.export import (
    ...
    try:
        ...
        from grandpa.telemetry.session import TelemetrySession
    ...
    try:
        from grandpa.evals.core.export import export_hf_dataset
...
def _run_from_config(
    ...
    *,
    ...
    from grandpa.evals.core.config import expand_suite, load_eval_config
...
def run(
    ...
    from grandpa.evals.core.types import RunConfig
...
def reparse_judge(jsonl_path, out_path, in_place, summary_out):
    ...
    from grandpa.evals.scorers.liveresearch import rescore_from_metadata
