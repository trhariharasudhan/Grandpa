...
def normalize_app_name(value: str) -> str:
    ...
    cleaned = re.sub(r"\b(shortcut|app|application)\b", " ", cleaned, flags=re.IGNORECASE)
...
def canonicalize_app_identity(display_name: str, executable_name: str = "") -> str:
    ...
    without_version = re.sub(r"\s+v?\d+(?:\.\d+)+(?:\.\d+)*\s*$", "", without_qualifiers, flags=re.IGNORECASE)
    ...
    if normalized.startswith("microsoft ") and normalized.removeprefix("microsoft ") in _CANONICAL_NAMES:
...
def resolve_app(query: str, apps: list[ApplicationInfo]) -> AppResolveResult:
    ...
    exact_name = [app for app in searchable if normalized == app.name or normalized == app.canonical_key]
    if len(exact_name) == 1:
        return AppResolveResult("found", (exact_name[0],), f"Found {exact_name[0].display_name}.", 1.0)
    if len(exact_name) > 1:
        preferred = _preferred_single_match(exact_name)
        if preferred is not None:
            return AppResolveResult("found", (preferred,), f"Found {preferred.display_name}.", 1.0)
        return AppResolveResult("ambiguous", tuple(exact_name[:8]), _ambiguous_message(exact_name), 1.0)

    exact_alias = [app for app in searchable if normalized in app.aliases]
    if len(exact_alias) == 1:
        return AppResolveResult("found", (exact_alias[0],), f"Found {exact_alias[0].display_name}.", 1.0)
    if len(exact_alias) > 1:
        preferred = _preferred_single_match(exact_alias)
        if preferred is not None:
            return AppResolveResult("found", (preferred,), f"Found {preferred.display_name}.", 1.0)
        return AppResolveResult("ambiguous", tuple(exact_alias[:8]), _ambiguous_message(exact_alias), 1.0)

    starts_with = [app for app in searchable if app.name.startswith(normalized) or any(alias.startswith(normalized) for alias in app.aliases)]
    if len(starts_with) == 1:
        return AppResolveResult("found", (starts_with[0],), f"Found {starts_with[0].display_name}.", 0.88)
    if len(starts_with) > 1:
        return AppResolveResult("ambiguous", tuple(starts_with[:8]), _ambiguous_message(starts_with), 0.88)

    contains = [app for app in searchable if normalized in app.name or any(normalized in alias for alias in app.aliases)]
    if len(contains) == 1:
        return AppResolveResult("found", (contains[0],), f"Found {contains[0].display_name}.", 0.8)
    if len(contains) > 1:
        return AppResolveResult("ambiguous", tuple(contains[:8]), _ambiguous_message(contains), 0.8)

    fuzzy: list[tuple[float, ApplicationInfo]] = []
    for app in searchable:
        candidates = (app.name, *app.aliases)
        score = max(difflib.SequenceMatcher(a=normalized, b=candidate).ratio() for candidate in candidates)
        if score >= FUZZY_THRESHOLD:
            fuzzy.append((score, app))
    fuzzy.sort(key=lambda item: item[0], reverse=True)
    if not fuzzy:
        return AppResolveResult("missing", (), f"I could not find an installed app named {query}. Try `grandpa apps scan`.")
    best_score = fuzzy[0][0]
    close = [app for score, app in fuzzy if best_score - score < 0.05]
    if len(close) == 1:
        return AppResolveResult("found", (close[0],), f"Found {close[0].display_name}.", best_score)
    return AppResolveResult("ambiguous", tuple(close[:8]), _ambiguous_message(close), best_score)
