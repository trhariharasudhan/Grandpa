...
def load_app_registry(*, store_path: Path = DEFAULT_APP_REGISTRY_PATH) -> list[ApplicationInfo]:
    ...
    if not _supported_schema(payload):
        logger.warning("Application cache schema is stale; run `grandpa apps refresh`: %s", path)
        return []
    raw_apps = _raw_app_list(payload)
    apps = [ApplicationInfo.from_dict(item) for item in raw_apps if isinstance(item, dict)]
    logger.info("Application cache loaded: %s apps from %s", len(apps), path)
    return apps


def save_app_registry(apps: list[ApplicationInfo], *, store_path: Path = DEFAULT_APP_REGISTRY_PATH) -> None:
...
def _supported_schema(payload: Any) -> bool:
    return isinstance(payload, dict) and payload.get("schema_version") == APP_REGISTRY_SCHEMA_VERSION
