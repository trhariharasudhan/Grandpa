...
logger = logging.getLogger("grandpa.server.agent_manager")
...
def _make_lightweight_system(
    ...
    try:
        ...
        try:
            ...
            from grandpa.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from grandpa.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import grandpa.channels  # noqa: F401
    except Exception:
        pass
    ...
    # Also try to import browser tools (not included in grandpa.tools.__init__)
    for _browser_mod in ("grandpa.tools.browser", "grandpa.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("grandpa.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "grandpa.tools.browser",
            "grandpa.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from grandpa.core.credentials import TOOL_CREDENTIALS
    from grandpa.core.registry import ChannelRegistry, ToolRegistry
...
def _build_deep_research_tools(
    ...
    from grandpa.tools.knowledge_sql import KnowledgeSQLTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from grandpa.mcp.transport import StdioTransport, StreamableHTTPTransport
    from grandpa.tools.mcp_adapter import MCPToolProvider
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from grandpa.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from grandpa.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from grandpa.channels.sendblue import (
                        SendBlueChannel,
                    )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from grandpa.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from grandpa.channels.slack_daemon import (
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from grandpa.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from grandpa.core.credentials import get_credential_status
