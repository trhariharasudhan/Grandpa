...
class CheckpointManager:
    ...
    def save_checkpoint(self, state: ProjectState, checkpoint_id: Optional[str] = None) -> Checkpoint:
        ...
        target_file.write_text(json.dumps(checkpoint.to_dict(), indent=2), encoding="utf-8")
        return checkpoint
    ...
    def load_checkpoint(self, checkpoint_id: str) -> Checkpoint:
        ...
        if not target_file.exists():
            raise FileNotFoundError(f"Checkpoint file '{checkpoint_id}' not found at {target_file}.")
    ...
    def validate_checkpoint(self, checkpoint: Checkpoint, current_branch: str, current_health: str) -> bool:
    ...
    def restore_checkpoint(self, checkpoint_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            state_file.write_text(json.dumps(checkpoint.state.to_dict(), indent=2), encoding="utf-8")
            return True, f"Successfully restored project state to checkpoint '{checkpoint_id}'."
        except Exception as exc:
