...
def classify_intent(goal_text: str) -> AgentIntent:
    ...
    if any(w in lower for w in ("stop", "cancel", "pause", "exit", "terminate", "quit")):
        if "sprint" not in lower:
            return AgentIntent.STOP_CANCEL

    if re.search(r"\b(hello|hi|hey|greetings)\b", lower) or "good morning" in lower or "good afternoon" in lower:
        return AgentIntent.GREETING

    if any(w in lower for w in ("time", "clock", "what day is it", "what's the time")) or re.search(r"\bdate\b", lower):
    ...
    if any(w in lower for w in ("browser", "webpage", "website", "url", "summarize", "page")):
        return AgentIntent.BROWSER

    if any(w in lower for w in ("screen", "vision", "see", "describe what is on")):
        return AgentIntent.VISION

    if any(w in lower for w in ("click", "type", "press", "move cursor", "keys", "hwnd", "window", "notepad", "chrome")):
    ...
    if any(w in lower for w in ("how are you", "who are you", "what is your name", "tell me", "joke")):
