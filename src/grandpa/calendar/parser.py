...
class CalendarParser:
    ...
    def _parse_setup(self, command: str) -> CalendarAction | None:
        ...
        if command in {"calendar disconnect", "google calendar disconnect", "disconnect calendar"}:
    ...
    def _parse_read(self, command: str, raw: str) -> CalendarAction | None:
        ...
        if command in {"calendar week", "show this week calendar", "show calendar this week"}:
            return CalendarAction("list", date_range="week")
        if command in {"calendar upcoming", "upcoming calendar events", "show upcoming events"}:
            return CalendarAction("upcoming", date_range="upcoming")
        match = re.fullmatch(r"(?:search calendar for|find calendar events about|find meetings about) (.+)", command)
        if match:
    ...
    def _parse_write(self, command: str, raw: str) -> CalendarAction | None:
        match = re.fullmatch(r"(?:create|schedule|add) (?:a )?(?:meeting|event|reminder)(?: (.+))?", command)
        if match:
            detail = raw[match.start(1) : match.end(1)] if match.group(1) else ""
            return CalendarAction("create", title=_title_from_detail(detail), start_text=detail, args={"raw_detail": detail})
        match = re.fullmatch(r"(?:move|reschedule|update) (?:my |the )?(?:meeting|event)(?: to )?(.+)", command)
        if match:
            detail = raw[match.start(1) : match.end(1)]
            return CalendarAction("update", query="meeting", start_text=detail, args={"raw_detail": detail})
        match = re.fullmatch(r"(?:cancel|delete) (?:my |the )?(?:tomorrow's |tomorrows )?(?:meeting|event)(?: (.+))?", command)
        if match:
            detail = raw[match.start(1) : match.end(1)] if match.group(1) else ""
            return CalendarAction("delete", query=detail or "meeting", date_range="tomorrow" if "tomorrow" in command else "")
    ...
    def _parse_freebusy(self, command: str, raw: str) -> CalendarAction | None:
        if command in {"show free time this afternoon", "calendar free", "show free time", "when am i free"}:
...
def _title_from_detail(detail: str) -> str:
    cleaned = re.sub(r"\b(?:tomorrow|today|at|on|for|friday|monday|tuesday|wednesday|thursday|saturday|sunday|\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\b", " ", detail, flags=re.I)
