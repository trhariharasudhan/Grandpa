...
class CalendarClient:
    ...
    def list_events(
        ...
        return tuple(
            self._event_from_api(item) for item in response.get("items", [])[:limit]
        )
    ...
    def update_event(
        ...
        *,
        ...
        if start_text:
            body["description"] = (
                f"Updated by Grandpa from natural time: {start_text}; duration {duration_minutes} minutes"
            )
        result = (
            service.events()
    ...
    def _event_from_api(self, data: dict[str, Any]) -> CalendarEventSummary:
        ...
        attendees = tuple(
            str(item.get("email") or item.get("displayName") or "")
        ...
        return CalendarEventSummary(
            ...
            location=self.safety.sanitize_text(
                str(data.get("location") or ""), limit=300
            ),
            attendees=attendees,
