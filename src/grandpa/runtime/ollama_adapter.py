"""Ollama backend adapter conforming to Grandpa ModelRuntime interface."""
...
from grandpa.runtime.interface import StreamChunk
from grandpa.runtime.utils import (
...
class OllamaBackendAdapter(BackendAdapter):
    """Adapter bridging Ollama REST API to Grandpa ModelRuntime interface."""
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if response_format is not None:
            from grandpa.runtime.interface import ResponseFormat
    ...
    def pull_model(self, model: str) -> Dict[str, Any]:
        ...
        if data.get("error"):
            raise RuntimeModelPullError(model, str(data["error"]))
        return {"model": model, "status": data.get("status", "success")}

    def delete_model(self, model: str) -> Dict[str, Any]:
        """Delete an Ollama model through the local Ollama HTTP API."""
        try:
            ...
            body = exc.response.text[:500] if exc.response else ""
            raise RuntimeError(
                f"Ollama delete failed with {exc.response.status_code}: {body}"
            ) from exc
