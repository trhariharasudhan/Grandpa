"""Generate 10s RAW, Clarity, and Presence audio files from the exact same F5 synthesis."""
...
def main():
    print("Initializing F5-TTS 10-second clarity test generation script...")
    ...
    raw_path = runtime_root / "outputs" / "grandpa_10s_RAW.wav"
    clarity_path = runtime_root / "outputs" / "grandpa_10s_clarity.wav"
    presence_path = runtime_root / "outputs" / "grandpa_10s_presence.wav"

    # Step 1: Check/Delete existing target files so we overwrite them cleanly
    for p in (raw_path, clarity_path, presence_path):
        if p.exists():
            p.unlink()
    ...
    # Step 3: Process Grandpa Clarity
    print("\n--- Processing Grandpa Clarity ---")
    ...
    clarity_path.write_bytes(processed_f_bytes)
    print(f"Clarity profile saved to: {clarity_path}")

    # Step 4: Process Grandpa Presence
    print("\n--- Processing Grandpa Presence ---")
    ...
    presence_path.write_bytes(processed_g_bytes)
    print(f"Presence profile saved to: {presence_path}")

    # Step 5: Measurements and Reporting
    print("\n==================== MEASUREMENTS & REPORT ====================")
    raw_metrics = measure_audio(ffmpeg_path, raw_path)
    clarity_metrics = measure_audio(ffmpeg_path, clarity_path)
    presence_metrics = measure_audio(ffmpeg_path, presence_path)
    ...
    print(f"  Duration: {clarity_metrics['duration']:.2f} seconds")
    print(f"  Loudness: {clarity_metrics['lufs']:.2f} LUFS")
    print(f"  True Peak: {clarity_metrics['true_peak']:.2f} dBTP")
    print(f"  Clipping: {'Yes' if clarity_metrics['true_peak'] > 0.0 else 'No'}")
    print(f"  Processing time: {proc_f_time:.4f} seconds")

    print(f"\nProfile G (Presence):")
    print(f"  Exact filter chain: {','.join(g_filters)}")
    print(f"  Duration: {presence_metrics['duration']:.2f} seconds")
    print(f"  Loudness: {presence_metrics['lufs']:.2f} LUFS")
    print(f"  True Peak: {presence_metrics['true_peak']:.2f} dBTP")
    print(f"  Clipping: {'Yes' if presence_metrics['true_peak'] > 0.0 else 'No'}")
