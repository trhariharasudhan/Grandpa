...
use grandpa_core::{GrandpaError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, GrandpaError> {
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                GrandpaError::Io(std::io::Error::other(e))
            })?;
        }
    ...
    pub fn in_memory() -> Result<Self, GrandpaError> {
        Self::new(Path::new(":memory:"))
    }

    pub fn save(&self, trace: &Trace) -> Result<(), GrandpaError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, GrandpaError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| {
                GrandpaError::Io(std::io::Error::other(
                    e.to_string(),
                ))
            })?;
    ...
    pub fn count(&self) -> Result<usize, GrandpaError> {
        let conn = self.conn.lock();
        let count: i64 = conn
