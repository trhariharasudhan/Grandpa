...
use grandpa_core::config::GrandpaConfig;
...
pub fn discover_engines(config: &GrandpaConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, GrandpaError> {
    ...
    match key.as_str() {
        ...
        other => Err(GrandpaError::Engine(
            grandpa_core::error::EngineError::ModelNotFound(format!(
                "Unknown engine: {}",
                other
            )),
        )),
...
mod tests {
    ...
    fn test_get_engine_static_ollama() {
        let config = GrandpaConfig::default();
