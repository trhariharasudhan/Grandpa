...
pub struct PyWorkflowGraph {
    inner: grandpa_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: grandpa_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => grandpa_workflow::NodeType::Tool,
            "condition" => grandpa_workflow::NodeType::Condition,
            "parallel" => grandpa_workflow::NodeType::Parallel,
            "loop" => grandpa_workflow::NodeType::Loop,
            "transform" => grandpa_workflow::NodeType::Transform,
            _ => grandpa_workflow::NodeType::Agent,
        };
        let node = grandpa_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = grandpa_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<grandpa_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(grandpa_workflow::WorkflowBuilder::new(name)),
        }
    }
