...
pub struct PySecretScanner {
    inner: grandpa_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: grandpa_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: grandpa_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: grandpa_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: grandpa_security::GuardrailsEngine<grandpa_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => grandpa_security::RedactionMode::Redact,
            "block" => grandpa_security::RedactionMode::Block,
            _ => grandpa_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: grandpa_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<grandpa_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = grandpa_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: grandpa_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: grandpa_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: grandpa_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: grandpa_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: grandpa_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: grandpa_security::RateLimiter::new(
                grandpa_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: grandpa_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: grandpa_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => grandpa_security::TaintLabel::Pii,
            "secret" => grandpa_security::TaintLabel::Secret,
            "user_private" => grandpa_security::TaintLabel::UserPrivate,
            "external" => grandpa_security::TaintLabel::External,
            _ => grandpa_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &grandpa_security::TaintSet::from_labels(&[taint_label]),
        );
    }
