...
use crate::RUNTIME;
use grandpa_agents::OjAgent;
use grandpa_engine::rig_adapter::RigModelAdapter;
use grandpa_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(grandpa_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(grandpa_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(grandpa_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl AgentEnum {
    ...
    async fn run(
        &self,
        input: &str,
        context: Option<&grandpa_core::AgentContext>,
    ) -> Result<grandpa_core::AgentResult, grandpa_core::GrandpaError> {
        match self {
...
fn make_adapter(engine_key: &str, model: &str) -> PyResult<DefaultAdapter> {
    let config = grandpa_core::GrandpaConfig::default();
    let engine = grandpa_engine::get_engine_static(&config, Some(engine_key))
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = grandpa_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(grandpa_tools::ToolExecutor::new(None, None));
        let agent = grandpa_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(grandpa_tools::ToolExecutor::new(None, None));
        let agent = grandpa_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = grandpa_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = grandpa_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(grandpa_tools::ToolExecutor::new(None, None));
        let agent = grandpa_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => grandpa_agents::MemoryExtraction::Scratchpad,
            "structured_json" => grandpa_agents::MemoryExtraction::StructuredJson,
            "none" => grandpa_agents::MemoryExtraction::None,
            _ => grandpa_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => grandpa_agents::ObservationCompression::Truncate,
            "none" => grandpa_agents::ObservationCompression::None,
            _ => grandpa_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => grandpa_agents::RetrievalStrategy::Keyword,
            "semantic" => grandpa_agents::RetrievalStrategy::Semantic,
            "none" => grandpa_agents::RetrievalStrategy::None,
            _ => grandpa_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => grandpa_agents::TaskDecomposition::Monolithic,
            "hierarchical" => grandpa_agents::TaskDecomposition::Hierarchical,
            _ => grandpa_agents::TaskDecomposition::Phased,
        };

        let monitor_config = grandpa_agents::MonitorConfig {
        ...
        let agent = grandpa_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: grandpa_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: grandpa_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
