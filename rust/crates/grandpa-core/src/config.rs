...
//!
//! Rust translation of `src/grandpa/core/config.py`.
...
/// Default config directory: `~/.grandpa/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".grandpa")
}

/// Default config file path: `~/.grandpa/config.toml`
...
fn default_sandbox_image() -> String { "Grandpa-sandbox:latest".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<GrandpaConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: GrandpaConfig = toml::from_str(&content)?;
        ...
        let mut cfg = GrandpaConfig::default();
        cfg.engine.default = recommended_engine;
        cfg
    };
...
mode = "block"
"#;
        let cfg: GrandpaConfig = toml::from_str(toml_str).unwrap();
