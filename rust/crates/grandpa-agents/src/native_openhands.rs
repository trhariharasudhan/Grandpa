...
use grandpa_core::{AgentContext, AgentResult, GrandpaError, ToolResult};
use grandpa_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        grandpa_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        grandpa_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    GrandpaError::Agent(grandpa_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<grandpa_engine::Engine>>;
