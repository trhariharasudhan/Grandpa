...
# Grandpa's self-hosted PostHog analytics backend.
#
...
#
# Usage:
#   sudo bash posthog-hetzner-prep.sh analytics.grandpa.ai you@Grandpa.ai
#
# After it finishes:
#   1. Visit https://<DOMAIN>/ and create the admin account.
#   2. Create project "Grandpa".
...
Examples:
  sudo bash posthog-hetzner-prep.sh analytics.grandpa.ai team@Grandpa.ai
...
Next steps:
  ...
  3. Create project "Grandpa".
  4. Settings → Project → Project API Key — copy the phc_… value.
  5. Update src/grandpa/core/config.py AnalyticsConfig defaults:
        host = "https://$DOMAIN"
  ...
  8. Ship a release of Grandpa with the new config defaults.
