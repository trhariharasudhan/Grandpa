...
A set of developer-focused scripts that use grandpa tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Grandpa` instance with the `native_react` (ReAct) agent
...
1. **Install grandpa** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/grandpa/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/grandpa/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run Grandpa eval list` or
inspect `src/grandpa/tools/` for the full registry.
