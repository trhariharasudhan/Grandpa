...
This tutorial demonstrates how to use grandpa to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Grandpa` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `Grandpa scheduler` CLI
...
- **Model selection**: Use `--model` to target a specific model, or let grandpa auto-select from what is available.
