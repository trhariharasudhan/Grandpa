...
grandpa orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting grandpa to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- grandpa installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the grandpa channel docs).
...
grandpa supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/grandpa/config.toml` for the full list of channel and agent
options.
...
this via the grandpa scheduler:

```bash
Grandpa scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/grandpa/recipes/data/operators/` for examples.
