...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: Grandpa channel list   to see available channels.\n",
    )
    ...
    click.echo("  from grandpa import Grandpa")
    click.echo(f'  j = Grandpa(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: Grandpa channel status  (to verify "{channel}" is connected)')
