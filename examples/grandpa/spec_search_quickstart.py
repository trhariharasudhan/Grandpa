...
are local fakes, so you can ``python examples/grandpa/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/grandpa/examples/spec-search-quickstart.toml``
via the regular ``grandpa.core.config.load_config`` machinery — you can copy
that TOML to ``~/.grandpa/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    grandpa_HOME=/tmp/grandpa-spec-search-demo \\
...
from grandpa.core.config import SpecSearchLearningConfig
from grandpa.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from grandpa.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from grandpa.learning.spec_search.multi_session import SpecSearchLoop
from grandpa.learning.spec_search.orchestrator import SpecSearchOrchestrator
from grandpa.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from grandpa.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = grandpa.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.grandpa/config.toml``.
    ...
    #
    # (after copying ``configs/grandpa/examples/spec-search-quickstart.toml``
    # to ``~/.grandpa/config.toml``)
    ...
    home = Path(
        os.environ.get("grandpa_HOME")
        or tempfile.mkdtemp(prefix="grandpa-spec-search-")
    )
    print(f"grandpa_HOME = {home}")
    ...
    with patch(
        "grandpa.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
