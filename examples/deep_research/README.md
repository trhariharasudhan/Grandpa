...
using grandpa. The assistant uses an orchestrator agent loop with web
...
- grandpa installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `Grandpa eval`
runner.
...
User query
  |
  v
Grandpa SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `Grandpa agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `grandpa.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/grandpa/recipes/) -- composable TOML configs that
