...
```bash
uv run Grandpa --version   # Should print 0.1.0
uv run Grandpa --help      # Show all subcommands
```
...
grandpa uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=grandpa --cov-report=html
```
...
grandpa uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/grandpa/`:

```
src/grandpa/
    __init__.py                 # Package root, __version__
    sdk.py                      # Grandpa class — high-level Python SDK

    core/                       # Shared infrastructure
        config.py               # GrandpaConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # Grandpa ask
        init_cmd.py             # Grandpa init
        model.py                # Grandpa model list/info
        memory_cmd.py           # Grandpa memory index/search/stats
        telemetry_cmd.py        # Grandpa telemetry stats/export/clear
        bench_cmd.py            # Grandpa bench run
        serve.py                # Grandpa serve
```
...
try:
    import grandpa.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
