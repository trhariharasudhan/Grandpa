...
through Pearl's vLLM miner and grandpa.

grandpa can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
grandpa only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and grandpa-specific conversion
...
`Grandpa mine start`, vLLM `/v1/models`, grandpa inference routing, Pearl
gateway template refresh, and `Grandpa mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in grandpa until we have clean
...
2. Convert the target model.
   ...
   grandpa includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `Grandpa mine inspect-model` and
   `Grandpa mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `Grandpa mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `Grandpa mine status` parses those metrics.

5. Promote the model in grandpa.
...
```text
src/grandpa/mining/_models.py
```

`Grandpa mine models` renders that registry. Planned models are visible to users
...
- `Grandpa mine inspect-model --model <pearl-model-id> --allow-planned`
- `Grandpa mine init --model <pearl-model-id>`
- `Grandpa mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `Grandpa ask "Say hello in one sentence."`
- `Grandpa mine status`
- `Grandpa mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `Grandpa mine validate-model --output` to the issue.
