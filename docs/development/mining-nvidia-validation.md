...
prove grandpa wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through grandpa.
...
- Docker container `grandpa-pearl-miner` starts.
- `~/.grandpa/runtime/mining.json` is written.
...
```bash
docker ps --filter name=grandpa-pearl-miner
cat ~/.grandpa/runtime/mining.json
uv run Grandpa mine logs --tail 200
...
## Verify grandpa Inference Uses Mining Endpoint

Run:

```bash
uv run Grandpa mine doctor
uv run Grandpa ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/grandpa/mining/_metrics.py`.
...
```bash
uv run Grandpa mine stop
docker ps --filter name=grandpa-pearl-miner
test ! -e ~/.grandpa/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `Grandpa ask` no longer routes through `vllm-pearl-mining` unless another
...
- grandpa inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run Grandpa mine doctor
uv run Grandpa mine status || true
uv run Grandpa mine logs --tail 300 || true
docker inspect grandpa-pearl-miner || true
