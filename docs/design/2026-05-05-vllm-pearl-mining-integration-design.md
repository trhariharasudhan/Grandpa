...
### In scope (v1)
- New `grandpa.mining` subsystem with `MiningProvider` ABC, `MinerRegistry`, `MiningCapabilities` / `MiningConfig` / `MiningStats` dataclasses
- `vllm-pearl` provider implementation: orchestrate Pearl's published `vllm-miner` Docker container
- `[mining]` TOML section in OJ config; `MiningConfig` field in `GrandpaConfig`
- New CLI namespace: `Grandpa mine init|start|stop|status|doctor|attach|logs`
- Runtime sidecar at `~/.grandpa/runtime/mining.json` for engine ↔ mining handoff
...
| Engine attachment | Runtime sidecar JSON at `~/.grandpa/runtime/mining.json` | Existing vLLM engine class stays untouched. Sidecar is the single source of truth tying mining lifecycle to engine resolution. Inspectable via `cat`. |
...
src/grandpa/cli/
    mine_cmd.py          # Grandpa mine init|start|stop|status|doctor|attach|logs
...
`MinerRegistry` added to `src/grandpa/core/registry.py` as a peer to `EngineRegistry`, `AgentRegistry`, etc. `tests/conftest.py`'s autouse `_clean_registries` fixture is updated to include `MinerRegistry.clear()`.
...
[mining.extra]
docker_image_tag         = "grandpa/pearl-miner:<pinned-ref>"
...
`~/.grandpa/runtime/mining.json` (created on `mine start`, removed on `mine stop`):
...
1. `Grandpa mine start` → `MinerRegistry.get("vllm-pearl").start(config)`.
...
4. `Grandpa ask` and the SDK route to that endpoint transparently. The user's normal inference is the mining work.
...
Power users running their own Pearl container skip `Grandpa mine start` and write the sidecar themselves via `Grandpa mine attach --vllm-endpoint=... --gateway-url=...`. Decouples lifecycle from wiring.
...
| Command | Purpose |
|---|---|
| `Grandpa mine init` | Interactive: hardware/Docker checks, prompt for wallet + pearld credentials, write `[mining]`, pull/build image. Does NOT start mining. Pre-checks `>=200 GB` free disk. |
| `Grandpa mine start` | Launch container via the registered provider, write sidecar, print endpoint info. Idempotent if running. |
| `Grandpa mine stop` | Stop container, remove sidecar. Idempotent if not running. |
| `Grandpa mine status` | Read sidecar + query gateway `:8339/metrics`. Print `MiningStats`. |
| `Grandpa mine doctor` | Capability matrix; every check ✓/✗ with reason. Works in any state. |
| `Grandpa mine attach` | Manual mode: write sidecar without launching. |
| `Grandpa mine logs [-f]` | Tail container logs through Docker SDK. |
...
Image
  grandpa/pearl-miner:<ref>   present (built 2026-04-30)   ✓
Pearl node
...
- Existing `com.grandpa.gateway` daemon is unchanged.
...
- launchd/systemd installation surface (`Grandpa daemon install`) untouched.

### 6.5 Concurrency

POSIX `flock` on `~/.grandpa/runtime/mining.lock` prevents racing `mine start` invocations.

### 6.6 `Grandpa ask` UX hint

When `[mining]` is configured but no sidecar exists, `cli/hints.py` emits one line: `"mining configured but not running — start it with \`Grandpa mine start\`"`. One-line UX nudge, no new infrastructure.
...
| **Build-from-pin** | OJ git-clones Pearl at a pinned ref into `~/.grandpa/cache/pearl/`, then `docker buildx build` | v1 default (Pearl publishes nothing today) |
...
- **`auto_remove=False`** so a crashed container stays around for `Grandpa mine logs` post-mortem.
...
- `Grandpa mine init` triggers `ensure_image()`, streams build/pull output through the CLI with a clear time estimate (`"Building Pearl miner image — first run takes ~45 min on a fast machine"`).
- `Grandpa mine doctor` reports `image: present (tag, age, sha)` or `image: missing (run mine init)`.
- `Grandpa mine prune` (v1.x) cleans old `grandpa/pearl-miner:*` tags. Manual `docker image rm` works in v1.
...
- **v1: on-demand only.** `Grandpa mine status` makes one HTTP GET per call (~10 ms). No background polling.
...
- `Grandpa telemetry stats --mining` (v1.x) joins to the latest `MiningStats` snapshot and reports `tokens / share`, `joules / share`, `est. PRL / kWh`.
...
**1. `submit_target` parsed into a tagged union; only one variant works.** `SoloTarget` accepted at runtime in v1; `PoolTarget` raises `NotImplementedError("pool support is v2 — track grandpa#XYZ")`. Reachable only by users who edit their config to opt in.
...
| Concurrent `mine start` | POSIX `flock` on `~/.grandpa/runtime/mining.lock`; second invocation errors clearly | clear message |
