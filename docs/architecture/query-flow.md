...
This page traces the end-to-end journey of a user query through the grandpa system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: Grandpa ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: GrandpaConfig (hardware, engine defaults)
...
grandpa supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
Grandpa ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.grandpa/config.toml
```
...
- Returns a `GrandpaConfig` dataclass with all settings
...
```python
from grandpa.learning import ensure_registered
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.grandpa/telemetry.db`.
...
The `Grandpa` class in `sdk.py` provides the same query flow through a Python API:

```python
from grandpa import Grandpa

j = Grandpa(model="qwen3:8b", engine_key="ollama")
