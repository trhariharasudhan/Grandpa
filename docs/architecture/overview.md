...
The sandbox module (`grandpa.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in grandpa use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/grandpa/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           GrandpaConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              Grandpa ask -- query the assistant
        serve.py            Grandpa serve -- start API server
