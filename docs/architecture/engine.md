...
The Engine primitive provides the **inference runtime** -- the layer that connects grandpa to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
grandpa automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from grandpa.engine import discover_engines
...
Engine hosts and defaults are configured in `~/.grandpa/config.toml` using **nested per-engine sub-sections**:
...
```python
from grandpa.engine._base import messages_to_dicts
