...
!!! note "Backward compatibility"
    The canonical locations are now `grandpa.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `grandpa.core.types` (for `RoutingContext`). The old `grandpa.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `grandpa.learning._stubs`.
...
```bash
Grandpa ask --router learned "What is the capital of France?"
```
...
```python
from grandpa.learning.router import HeuristicRouter, build_routing_context
...
```python
from grandpa.learning.router import build_routing_context
...
```python
from grandpa.learning.trace_policy import TraceDrivenPolicy
...
```python
from grandpa.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from grandpa.learning.sft_policy import SFTPolicy
```
...
```python
from grandpa.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from grandpa.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from grandpa.learning.heuristic_reward import HeuristicRewardFunction
...
over grandpa's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `grandpa-learning`
