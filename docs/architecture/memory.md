...
- **Persistence:** Data persists across restarts in `~/.grandpa/memory.db`
...
!!! note "Backward compatibility"
    The old imports (e.g., `from grandpa.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `grandpa.tools.storage.*`.
...
```python
from grandpa.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from grandpa.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from grandpa.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.grandpa/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
