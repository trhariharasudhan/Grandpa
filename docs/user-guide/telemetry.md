...
grandpa has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.grandpa/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.grandpa/telemetry.db")
...
Grandpa telemetry stats -n 5          # Top 5 models only

# Export records
Grandpa telemetry export              # JSON to stdout
Grandpa telemetry export -f csv       # CSV to stdout
Grandpa telemetry export -o data.json # JSON to file
Grandpa telemetry export -f csv -o metrics.csv

# Clear all records
Grandpa telemetry clear               # With confirmation prompt
Grandpa telemetry clear --yes         # Without confirmation
```
