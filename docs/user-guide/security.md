...
grandpa includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.
...
=== "Warn mode (default)"
    ...
    from grandpa.security.guardrails import GuardrailsEngine
    from grandpa.security.types import RedactionMode
...
=== "Block mode"
    ...
    from grandpa.engine.ollama import OllamaEngine
    from grandpa.security.guardrails import GuardrailsEngine, SecurityBlockError
    from grandpa.security.types import RedactionMode
    from grandpa.core.types import Message, Role
...
from grandpa.security.guardrails import GuardrailsEngine
from grandpa.security.scanner import SecretScanner
from grandpa.security.types import RedactionMode
...
from grandpa.security.scanner import PIIScanner
...
from grandpa.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.grandpa/audit.db", bus=bus)
...
import time
from grandpa.security.audit import AuditLogger
from grandpa.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.grandpa/audit.db")
...
Security settings live in the `[security]` section of `~/.grandpa/config.toml`.
...
- [API Reference: Security](../api-reference/grandpa/security/index.md) — full class and function signatures
