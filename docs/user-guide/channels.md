...
The channels module lets grandpa send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from grandpa.channels.telegram import TelegramChannel
...
from grandpa.channels.discord_channel import DiscordChannel
...
from grandpa.channels.slack import SlackChannel
...
The `Grandpa channel` subcommand group provides quick access to channel operations.
...
Grandpa channel send telegram "Build completed successfully"
```
...
When `Grandpa serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.grandpa/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.grandpa/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from grandpa!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  grandpa
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. grandpa sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since grandpa runs locally, you need a tunnel so SendBlue can reach your webhook:
...
# Send a message
ok = channel.send("+15551234567", "Hello from grandpa!")
```
...
Grandpa channel status --channel-type sendblue

# Send a message
Grandpa channel send sendblue "+15551234567" "Hello from Grandpa!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `Grandpa serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.grandpa/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from grandpa.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Grandpa",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.grandpa/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.grandpa/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Grandpa"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.grandpa/whatsapp_baileys_bridge/auth"
...
- [API Reference: Channels](../api-reference/grandpa/channels/index.md) -- full class and type signatures
