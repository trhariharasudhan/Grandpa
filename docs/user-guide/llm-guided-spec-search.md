...
```bash
python examples/grandpa/spec_search_quickstart.py
```
...
`grandpa.learning.spec_search.composite_reward.score_batch`.
...
`configs/grandpa/examples/spec-search-quickstart.toml`. Copy it to
`~/.grandpa/config.toml` (or set `grandpa_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `grandpa.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `grandpa.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `grandpa.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `grandpa.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `grandpa.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/grandpa/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `grandpa.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from grandpa.evals.datasets.adp import ADPDataset
from grandpa.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.grandpa/traces.db")
...
`src/grandpa/evals/backends/grandpa_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.grandpa/traces.db` regardless of caller
...
- `examples/grandpa/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/grandpa/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/grandpa/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/grandpa/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/grandpa/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/grandpa/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/grandpa/learning/spec_search/external_adapter.py` — corpus → trace adapter.
