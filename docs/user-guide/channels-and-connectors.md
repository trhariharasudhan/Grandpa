...
grandpa has two types of integrations:
...
```json
{
    "display_information": { "name": "grandpa" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "grandpa", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "grandpa" |
...
Slack serves two purposes in grandpa:
...
**DM your agent:**
- In Slack, find **grandpa** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "grandpa"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @grandpa` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @grandpa` in the channel |
...
3. **Connect in grandpa:**
   ...
   - CLI: `uv run Grandpa connect notion`
...
3. **Connect in grandpa:**
   ...
   - CLI: `uv run Grandpa connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the grandpa desktop app
...
3. **Connect in grandpa:**
   ...
   - CLI: `uv run Grandpa connect outlook`
...
2. **Connect in grandpa:**
   - Desktop/Browser: Agents → Channels tab → Obsidian → paste the vault path
   - CLI: `uv run Grandpa connect obsidian --path /path/to/vault`
...
4. **Connect in grandpa:**
   ...
   - CLI: `uv run Grandpa connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run Grandpa deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.grandpa/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.grandpa/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
