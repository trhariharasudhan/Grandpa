...
Agents are the agentic logic layer of grandpa. They determine how a query is processed -- whether it goes directly to a model, through a tool-calling loop, via ReAct reasoning, CodeAct code execution, recursive decomposition, or an external agent runtime. All agents implement the `BaseAgent` ABC and are registered via the `AgentRegistry`.
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from grandpa.agents.react import ReActAgent` also still works.
...
```bash
# Simple agent
Grandpa ask --agent simple "What is the capital of France?"

# Orchestrator with tools
Grandpa ask --agent orchestrator --tools calculator,think "What is sqrt(256)?"

# NativeReActAgent
Grandpa ask --agent native_react --tools calculator "What is 2+2?"

# ReAct alias (same as native_react)
Grandpa ask --agent react --tools calculator,think "Solve step by step: 15% of 340"

# NativeOpenHandsAgent
Grandpa ask --agent native_openhands --tools calculator,web_search "Summarize example.com"

# RLMAgent
Grandpa ask --agent rlm "Summarize this long document"

# OpenHands SDK agent
Grandpa ask --agent openhands "Fix the bug in test_utils.py"
```
...
!!! warning "Requirements"
    Requires Node.js 22+ on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.grandpa/claude_code_runner/` on first use via `npm install`.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.grandpa/claude_code_runner/` and runs `npm install --production` if `node_modules` is missing.
...
4. The Python side parses the output between `---grandpa_OUTPUT_START---` and `---grandpa_OUTPUT_END---` markers, extracting content, tool results, and metadata.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what grandpa tool-calling agents support.

```python
from grandpa.agents.claude_code import ClaudeCodeAgent
...
```bash
# Via CLI
Grandpa ask --agent claude_code "Refactor the tests to use pytest fixtures"
```

!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept grandpa tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.
...
```bash
# Via CLI
Grandpa ask --agent operative "Check system status"
```
...
```bash
# Via CLI
Grandpa ask --agent monitor_operative "Analyze the security audit findings"
```
...
```python
from grandpa.sandbox import ContainerRunner, SandboxedAgent
from grandpa.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="grandpa-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/grandpa/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"grandpa-sandbox:latest"`| Docker image to run                            |
