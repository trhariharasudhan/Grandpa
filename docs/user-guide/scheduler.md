...
!!! note "Optional component"
    The scheduler is a standalone module (`grandpa.scheduler`). It is not wired into the default `Grandpa` / `GrandpaSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `Grandpa scheduler start`.
...
The `Grandpa scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
Grandpa scheduler list --status active

# Paused tasks
Grandpa scheduler list --status paused
```
...
Grandpa scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
Grandpa scheduler resume a3f9b12c4d8e
```
...
Grandpa scheduler cancel a3f9b12c4d8e
```
...
Grandpa scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.grandpa/scheduler.db")  # (1)!

# Wire in a GrandpaSystem for task execution
...
4. Starts a daemon thread named `"Grandpa-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Grandpa ask --agent orchestrator \
...
Scheduler settings live in the `[scheduler]` section of `~/.grandpa/config.toml`.

```toml title="~/.grandpa/config.toml"
[scheduler]
enabled = false
db_path = "~/.grandpa/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.grandpa/scheduler.db`     | SQLite database path                       |
