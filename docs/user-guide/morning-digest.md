...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.grandpa/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.grandpa/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.grandpa/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." Grandpa digest --fresh
```
...
```bash
Grandpa digest --fresh          # Generate a new digest now
Grandpa digest                  # Show today's cached digest
Grandpa digest --text-only      # Print text without audio
Grandpa digest --history        # Show past digests
Grandpa digest --schedule "0 6 * * *"   # Set daily schedule
Grandpa digest --schedule off   # Disable schedule
Grandpa digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Grandpa (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/grandpa/prompts/personas/{name}.md`. The default `Grandpa` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.grandpa/connectors/news_rss.json`:
...
```bash
Grandpa serve  # Start the server
...
**"No digest for today"** — Run `Grandpa digest --fresh` to generate one.

**Empty sections** — Check connector status with `Grandpa connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
