...
grandpa ships with two benchmarks:
...
from grandpa.bench.throughput import ThroughputBenchmark
...
Grandpa bench run -n 50

# Run only the latency benchmark
Grandpa bench run -b latency

# Run only the throughput benchmark with 20 samples
Grandpa bench run -b throughput -n 20

# Specify model and engine
Grandpa bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
Grandpa bench run --json

# Write JSONL results to a file
Grandpa bench run -o results.jsonl

# Combine options
Grandpa bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from grandpa.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
Grandpa bench run -b context_length
```
