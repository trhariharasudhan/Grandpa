...
# Execute a tool call
from grandpa.core.types import ToolCall
...
```python
from grandpa.tools._stubs import build_tool_descriptions
...
```python
from grandpa.tools.retrieval import RetrievalTool
from grandpa.memory.sqlite import SQLiteMemory
...
```python
from grandpa.scheduler.tools import ScheduleTaskTool
from grandpa.scheduler.scheduler import TaskScheduler
from grandpa.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.grandpa/scheduler.db")
scheduler = TaskScheduler(store=store, system=Grandpa_system)
scheduler.start()
...
```bash
Grandpa ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
Grandpa ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
Grandpa ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
