...
!!! note "Before you begin"
    This tutorial assumes grandpa is installed with Ollama running and a model available (e.g., `qwen3.5:9b`). If you have not completed setup yet, start with the [Quick Start guide](../getting-started/quickstart.md).

## Step 1: Install Skills from Hermes Agent

grandpa can import skills from the [Hermes Agent](https://github.com/NousResearch/hermes-agent) skill library maintained by NousResearch. Let's install a few useful ones.

```bash
# Install individual skills
Grandpa skill install hermes:arxiv
Grandpa skill install hermes:github-pr-workflow

# Or bulk install an entire category
Grandpa skill sync hermes --category research
```

The first install clones the Hermes repo to `~/.grandpa/skill-cache/hermes/` (one-time, ~5s). Subsequent installs reuse the cache.
...
```bash
cat ~/.grandpa/skills/hermes/arxiv/SKILL.md | head -40
```

The `.source` file records provenance:

```bash
cat ~/.grandpa/skills/hermes/arxiv/.source
```
...
```bash
Grandpa ask "Use the code-explainer skill to explain this Python code: for i in range(5): print(i*2)" \
...
```bash
Grandpa ask "Use the math-solver skill to compute 17 * 23" \
  --engine ollama --model qwen3.5:9b
```
...
```bash
mkdir -p ~/.grandpa/skills/my-reviewer
```

Write a SKILL.md:

```bash
cat > ~/.grandpa/skills/my-reviewer/SKILL.md << 'EOF'
---
name: my-reviewer
...
```bash
Grandpa ask "Use the my-reviewer skill to review this function: def login(user, pwd): return db.query(f'SELECT * FROM users WHERE name={user} AND pass={pwd}')" \
  --engine ollama --model qwen3.5:9b
```
...
Grandpa ask "Use math-solver to compute 100 / 7"
Grandpa ask "Use code-explainer to explain: lambda x: x**2"
Grandpa ask "Use my-reviewer to review: def add(a,b): return a+b"
Grandpa ask "Use math-solver to compute 2**10"
Grandpa ask "Use code-explainer to explain: [x for x in range(10) if x % 2 == 0]"
```

Each query produces a trace in `~/.grandpa/traces.db` with skill metadata tags (`skill`, `skill_source`, `skill_kind`).
...
Grandpa skill discover --dry-run --min-frequency 2

# Write discovered patterns as skill manifests
Grandpa skill discover --min-frequency 2
```
...
This produces overlay files at `~/.grandpa/learning/skills/<skill-name>/optimized.toml` with improved descriptions and few-shot examples extracted from your best traces.

Inspect what was produced:

```bash
Grandpa skill show-overlay math-solver
Grandpa skill show-overlay code-explainer
```
...
For a hands-off experience, add this to `~/.grandpa/config.toml`:
...
|---------|-------------|
| **Installing skills** | `Grandpa skill install hermes:arxiv` — imported from public sources |
| **Using skills** | `Grandpa ask "Use the code-explainer skill..."` — agent invokes skills as tools |
...
| **Discovering patterns** | `Grandpa skill discover` — mined traces for recurring tool sequences |
| **Optimizing skills** | `Grandpa optimize skills --policy dspy` — improved descriptions + few-shot examples |
| **Benchmarking** | `Grandpa bench skills` — measured the impact across 4 conditions |
