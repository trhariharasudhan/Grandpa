...
1. Both `model` and `engine_key` are optional. Omitting them uses auto-detected defaults from `~/.grandpa/config.toml`.
...
4. The tool list is passed directly. Any registered tool name is valid — run `Grandpa agent info native_react` to see all available tools.
