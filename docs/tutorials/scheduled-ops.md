...
Each script follows the same SDK pattern: create a `Grandpa` instance, call `j.ask()` with an agent and tools, print the result, and close the instance. The schedule is managed externally by the grandpa scheduler daemon.
...
```mermaid
graph TD
    A[Grandpa scheduler start] --> B[Scheduler Daemon]
...
Register each script as a recurring task using `Grandpa scheduler create`:
...
Grandpa scheduler create "Check gym schedule" \
...
!!! note "Cron expression syntax"
    grandpa uses standard five-field cron syntax: `minute hour day-of-month month day-of-week`. Install `croniter` (`uv add croniter`) for full expression support including ranges and step values. Without it, basic `hour:minute` patterns still work.
...
from grandpa.scheduler import TaskScheduler
...
j = Grandpa()  # uses defaults from ~/.grandpa/config.toml
response = j.ask(
...
To route script output to Slack or any other supported channel, pipe stdout through `Grandpa channel send`:

```bash title="Terminal"
uv run python examples/scheduled_ops/daily_digest.py \
    --topics "AI,finance" | Grandpa channel send slack
```
...
!!! warning "Channel credentials"
    Live channel output requires channel-specific credentials. Run `Grandpa add slack` (or the relevant provider) to set up the MCP server and credential store, then configure environment variables in your `.env` file before starting the scheduler daemon.
...
- **One-time tasks**: Use `--type once --value "2026-04-01T09:00:00"` with `Grandpa scheduler create` for non-recurring tasks.
