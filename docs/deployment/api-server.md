...
grandpa includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
The server reads defaults from `~/.grandpa/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
!!! warning "Server dependency check"
    If the `[server]` extra is not installed, `Grandpa serve` exits with a clear error message explaining how to install the required dependencies.
...
The `[server]` section of `~/.grandpa/config.toml` controls default server behavior:
...
CLI flags override config file values. For example, `Grandpa serve --port 9000` overrides the `port` setting in the config file.
...
For production deployments, run grandpa behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
...
server {
    listen 443 ssl;
    server_name Grandpa.example.com;

    ssl_certificate /etc/ssl/certs/Grandpa.pem;
    ssl_certificate_key /etc/ssl/private/Grandpa.key;
...
```bash
Grandpa serve --host 127.0.0.1 --port 8000
```
