...
|---|----------|-------|-----------------|------|
| 1 | Template launch | `Grandpa agents launch`, pick research_monitor | Agent created, config printed with curated tools | [ ] |
| 2 | Manual run | `Grandpa agents run <id>` | Output shows reasoning + tool calls, status -> idle | [ ] |
| 3 | Immediate ask | `Grandpa agents ask <id> "summarize recent AI news"` | Synchronous response in terminal | [ ] |
| 4 | Queued instruct | `Grandpa agents instruct <id> "focus on diffusion"`, then `Grandpa agents run <id>` | Queued -> delivered, response in `Grandpa agents messages <id>` | [ ] |
| 5 | Status check | `Grandpa agents status` after 3+ runs | total_runs, total_cost, last_run_at populated | [ ] |
| 6 | Pause/resume | `Grandpa agents pause <id>`, verify skipped, `Grandpa agents resume <id>`, verify fires | Status toggles correctly | [ ] |
| 7 | Daemon scheduling | `Grandpa agents daemon` with interval agent (60s) | 3+ ticks fire on schedule, memory accumulates | [ ] |
| 8 | Budget exhaustion | Set max_cost=0.001, run until exceeded | Status becomes budget_exceeded | [ ] |
| 9 | Error recovery | Kill Ollama mid-tick, then `Grandpa agents recover <id>` | Error -> recover -> idle with checkpoint | [ ] |
