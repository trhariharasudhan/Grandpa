---
title: Configuration
description: Complete reference for grandpa configuration
---

# Configuration

grandpa uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
grandpa creates the `~/.grandpa/` directory and populates it with a default config when you run `Grandpa init`.
...
3. Writes `~/.grandpa/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `Grandpa init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, grandpa automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, grandpa uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
Grandpa ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.grandpa/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `Grandpa serve`.
...
```bash
Grandpa serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.grandpa/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.grandpa/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.grandpa/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.grandpa/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `Grandpa init`, grandpa probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.grandpa/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure grandpa entirely from Python without a TOML file:
...
```python
j = Grandpa(config_path="/path/to/my-config.toml")
```
...
grandpa respects the following environment variables:
