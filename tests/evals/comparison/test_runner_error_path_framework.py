...
default of ``"grandpa"`` to silently mislabel data from foreign
...
from grandpa.evals.core.dataset import DatasetProvider
from grandpa.evals.core.runner import EvalRunner
from grandpa.evals.core.scorer import Scorer
from grandpa.evals.core.types import EvalRecord, RunConfig
...
class TestErrorPathFrameworkPropagation:
    def test_failed_task_keeps_backend_framework_name(self, tmp_path: Any) -> None:
        ...
        backend's framework_name, not the dataclass default 'grandpa'."""
        cfg = RunConfig(
            benchmark="mock",
            backend="hermes",
            model="mock-model",
        ...
        # NOT from the EvalResult dataclass default of "grandpa".
    ...
    def test_default_framework_when_backend_missing_attr(self, tmp_path: Any) -> None:
        ...
        a sensible default (the ABC's "grandpa"), via getattr fallback."""
        ...
        # the conservative "grandpa" tag rather than crashing.
        assert results[0].framework == "grandpa"
