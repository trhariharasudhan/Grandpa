"""Verify GrandpaDirectBackend.generate_full returns the spec §6.2 extended fields."""
...
class TestGrandpaDirectExtendedFields:
    def test_generate_full_includes_framework_and_commit(self) -> None:
        ...
        # engine); set required attrs directly. GrandpaDirectBackend.generate_full
        # calls ``self._system.engine.generate(messages, ...)`` so we mock the
        # whole ``_system`` chain.
        backend = GrandpaDirectBackend.__new__(GrandpaDirectBackend)
        ...
        assert result["framework"] == "grandpa"
