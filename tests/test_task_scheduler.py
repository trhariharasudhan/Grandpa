...
def test_create_and_list_morning_routine(tmp_path: Path, monkeypatch) -> None:
    ...
    created = handle_scheduler_command(
        "create a morning routine", store=store, execute=False
    )
    listed = handle_scheduler_command(
        "what routines do I have?", store=store, execute=False
    )
...
def test_every_morning_open_allowlisted_apps(tmp_path: Path, monkeypatch) -> None:
    ...
    result = handle_scheduler_command(
        "Every morning open Chrome and VS Code", store=store, execute=False
    )
...
def test_reminder_every_hour(tmp_path: Path, monkeypatch) -> None:
    ...
    result = handle_scheduler_command(
        "remind me every hour to drink water", store=store, execute=False
    )
...
def test_disable_routine(tmp_path: Path, monkeypatch) -> None:
    ...
    result = handle_scheduler_command(
        "disable my morning routine", store=store, execute=False
    )
...
def test_unrelated_question_falls_back(tmp_path: Path) -> None:
    result = handle_scheduler_command(
        "What is Python?", store=_store(tmp_path), execute=False
    )
