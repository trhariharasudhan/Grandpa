...
def test_local_action_endpoint_emergency_stop_cancels_pending(
    client: TestClient, tmp_path: Path
):
    target = tmp_path / "delete-me.txt"
    target.write_text("x", encoding="utf-8")
...
def test_local_action_audit_endpoint_returns_redacted_entries(
    ...
    monkeypatch.setitem(
        ...
        type(
            ...
            (),
            {
                "copy": staticmethod(lambda _text: None),
                "paste": staticmethod(lambda: "secret"),
            },
        )(),
    )

    client.post(
        "/api/local-action", json={"action_type": "clipboard_write", "target": "secret"}
    )
