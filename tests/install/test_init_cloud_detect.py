"""Tests for init's cloud auto-detect and from-bare-Grandpa flag."""
...
def test_init_accepts_from_bare_Grandpa_flag(
    tmp_grandpa_home: Path, monkeypatch
) -> None:
    """The --from-bare-Grandpa flag exists and suppresses the launch-chat prompt."""
    ...
    result = runner.invoke(
        init,
        ["--from-bare-Grandpa", "--engine", "ollama", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
    # When --from-bare-Grandpa, we expect NOT to see a "launch chat" affordance.
...
def test_init_from_bare_Grandpa_skips_engine_prompt(
    tmp_grandpa_home: Path, monkeypatch
) -> None:
    """--from-bare-Grandpa must not hang on the engine-selection prompt
    ...
    result = runner.invoke(
        init,
        ["--from-bare-Grandpa", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
