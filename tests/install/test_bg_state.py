"""Tests for grandpa.cli._bg_state."""
...
def test_get_status_rust_built(tmp_grandpa_home: Path) -> None:
    (tmp_grandpa_home / ".state" / "extension-built").write_text("")
    s = _bg_state.get_status()
    assert s.rust_extension == "ready"


def test_get_status_rust_failed(tmp_grandpa_home: Path) -> None:
    (tmp_grandpa_home / ".state" / "extension-failed").write_text("error tail here")
    s = _bg_state.get_status()
...
def test_get_status_handles_deleted_file_race(tmp_grandpa_home: Path) -> None:
    """File present at glob-time, gone at read-time = treat as missing."""
    f = tmp_grandpa_home / ".state" / "extension-failed"
    f.write_text("oops")
