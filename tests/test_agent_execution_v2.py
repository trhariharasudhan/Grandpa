...
def test_command_catalogue() -> None:
    # 1. Allowed
    assert is_command_allowed(["python", "-m", "compileall", "-q", "src", "tests", "scripts"])
    ...
    assert not is_command_allowed(["uv", "run", "pytest", "tests/test.py", "|", "grep", "foo"])
...
def test_evidence_and_placeholder_validation(temp_workspace) -> None:
    ...
    changes = [{"path": str(file_path), "diff": "--- a/lib.py\n+++ b/lib.py\n@@ -1,2 +1,2 @@\n def my_func():\n-    retur 42\n+    return 42\n"}]
    ...
    changes_placeholder = [{"path": str(file_path), "diff": "--- a/lib.py\n+++ b/lib.py\n@@ -1,2 +1,2 @@\n def my_func():\n-    retur 42\n+# verified patch change\n"}]
    with pytest.raises(ValueError, match="no real changes"):
        build_patch_proposal("goal", analysis_ok, changes_placeholder, str(temp_workspace))

    changes_comment = [{"path": str(file_path), "diff": "--- a/lib.py\n+++ b/lib.py\n@@ -1,2 +1,2 @@\n def my_func():\n+# some comment\n"}]
    with pytest.raises(ValueError, match="no real changes"):
...
def test_calculator_fixture_end_to_end(temp_workspace) -> None:
    ...
    report = runtime.apply_patch(prop.proposal_id, str(temp_workspace), db_path=str(app_db))
...
def test_read_only_inspections_and_no_failures(temp_workspace, setup_memory) -> None:
    ...
    (temp_workspace / "tests" / "test_dummy.py").write_text("def test_dummy():\n    assert True\n", encoding="utf-8")

    runtime = AgentRuntime()

    # 1. Repository status inspection goal -> should produce no patch, skip pytest
    res_inspect = runtime.diagnose("Check Grandpa repository status", str(temp_workspace))
    ...
    res_ok = runtime.diagnose("Find the failing test in tests/test_dummy.py", str(temp_workspace))
...
def test_cli_execution(temp_workspace, setup_memory) -> None:
    ...
    # 1. Inspect command
    res_inspect = runner.invoke(agent_group, ["inspect", "Check status", "--workspace", str(temp_workspace)])
    ...
    res_diagnose = runner.invoke(
        agent_group,
        ["diagnose", "Fix calculator.py in tests/test_calculator.py", "--workspace", str(temp_workspace), "--db-path", str(app_db)]
    )
    ...
    # 3. Preview command
    res_preview = runner.invoke(agent_group, ["patch", "preview", "--db-path", str(app_db)])
    assert res_preview.exit_code == 0
    assert prop_id in res_preview.output

    # 4. Approve command
    res_approve = runner.invoke(agent_group, ["patch", "approve", prop_id, "--db-path", str(app_db)])
    ...
    res_apply = runner.invoke(
        agent_group,
        ["patch", "apply", prop_id, "--workspace", str(temp_workspace), "--db-path", str(app_db)]
    )
