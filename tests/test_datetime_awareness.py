...
from grandpa.core_ai_brain import BrainAnalysis, build_brain_context
...
def test_date_intent_variations(mock_frozen_time):
    ...
    assert (
        handle_datetime_intent("What is today's date?")
        == "Today is Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("what date is it today?")
        == "Today is Sunday, August 2, 2026."
    )
    ...
    # Day queries
    assert (
        handle_datetime_intent("What day is it today?")
        == "Today is Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("what day today?") == "Today is Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("Which day today?") == "Today is Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("what is that day today?")
        == "Today is Sunday, August 2, 2026."
    )
...
def test_time_intent_variations(mock_frozen_time):
    assert (
        ...
        == "It is 6:30 PM on Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("what time is it?")
        == "It is 6:30 PM on Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("current timing")
        == "It is 6:30 PM on Sunday, August 2, 2026."
    )
    assert (
        handle_datetime_intent("current time")
        == "It is 6:30 PM on Sunday, August 2, 2026."
    )
...
def test_year_and_month_intents(mock_frozen_time):
    ...
    assert (
        handle_datetime_intent("which month is this?") == "The current month is August."
    )
...
def test_user_dispute_override(mock_frozen_time):
    ...
    assert (
        response
        == "According to this computer's system clock, today is Sunday, August 2, 2026."
    )

    response = handle_datetime_intent(
        "No, that date is wrong. Today is August 15, 2023."
    )
    assert (
        response
        == "According to this computer's system clock, today is Sunday, August 2, 2026."
...
def test_whisper_prompt_echo():
    ...
    assert (
        is_prompt_echo("Grandpa assistant. Ollama. The current year may be 2026.")
        is True
    )
