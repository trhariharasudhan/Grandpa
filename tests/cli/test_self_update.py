"""Smoke tests for `Grandpa self-update`.
...
from grandpa.cli._install_detect import InstallInfo
...
def _mock_info(kind: str = "pypi") -> InstallInfo:
    return InstallInfo(
        kind=kind,
        upgrade_command={
            "pypi": "pip install --upgrade grandpa",
            "uv-tool": "uv tool upgrade grandpa",
            "editable-git": "cd /tmp/repo && git pull && uv sync",
            "unknown": "pip install --upgrade grandpa",
        }[kind],
    )
...
def test_check_flag_prints_command_and_exits_clean():
    ...
    assert "pip install --upgrade grandpa" in result.output
...
def test_yes_skips_confirmation_and_runs():
    ...
    assert args[0] == ["pip", "install", "--upgrade", "grandpa"]
