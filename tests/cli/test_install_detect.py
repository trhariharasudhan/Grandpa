...
from grandpa.cli._install_detect import InstallInfo, detect_install
...
def test_uv_tool_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "uv tool upgrade grandpa"
...
def test_pypi_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "pip install --upgrade grandpa"
...
def test_missing_grandpa_file_falls_back_to_pypi(monkeypatch):
    """grandpa unimportable / no __file__ — still get a sane default."""
