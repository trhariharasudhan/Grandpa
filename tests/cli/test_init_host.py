"""Tests for ``Grandpa init --host`` option."""
...
from grandpa.core.config import generate_default_toml, generate_minimal_toml
...
class TestInitHost:
    def test_init_host_writes_to_config(self, tmp_path: Path) -> None:
        """Grandpa init --host writes the host into config.toml."""
    ...
    def test_init_host_probes_and_reports(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            mock.patch("grandpa.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("grandpa.cli.init_cmd.PrivacyScanner"),
            mock.patch("grandpa.cli.init_cmd.httpx") as mock_httpx,
        ):
    ...
    def test_init_without_host_still_works(self, tmp_path: Path) -> None:
        """Grandpa init without --host still produces valid config."""
        config_dir = tmp_path / ".grandpa"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("grandpa.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
