"""Tests for the Python SDK — Grandpa class and MemoryHandle."""
...
from grandpa.sdk import Grandpa, MemoryHandle
...
class TestGrandpaInit:
    def test_default_config(self):
        j = Grandpa(config=GrandpaConfig())
        assert j.config is not None
        j.close()

    def test_custom_config(self):
        cfg = GrandpaConfig()
        j = Grandpa(config=cfg)
        assert j.config is cfg
        j.close()

    def test_version_property(self):
        j = Grandpa(config=GrandpaConfig())
        assert j.version == grandpa.__version__
        j.close()

    def test_engine_key_override(self):
        j = Grandpa(config=GrandpaConfig(), engine_key="custom")
        assert j._engine_key == "custom"
        j.close()

    def test_model_override(self):
        j = Grandpa(config=GrandpaConfig(), model="my-model")
        assert j._model_override == "my-model"
        j.close()


class TestGrandpaAsk:
    def test_ask_returns_string(self):
        engine = _make_engine("The answer is 42.")
        with patch("grandpa.sdk.get_engine", return_value=("mock", engine)):
            j = Grandpa(config=GrandpaConfig(), model="test-model")
    ...
    def test_ask_no_engine_raises(self):
        with patch("grandpa.sdk.get_engine", return_value=None):
            j = Grandpa(config=GrandpaConfig())
