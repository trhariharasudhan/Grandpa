...
class TestModelSecurity:
    def test_valid_gguf_filename(self) -> None:
        ...
        assert validate_gguf_filename("qwen2.5-0.5b-instruct-q4_k_m.GGUF") == "qwen2.5-0.5b-instruct-q4_k_m.GGUF"
...
class TestHuggingFaceModelSource:
    def test_resolve_composite_string(self) -> None:
        ...
        artifact = source.resolve("Qwen/Qwen2.5-0.5B-Instruct-GGUF/qwen2.5-0.5b-instruct-q4_k_m.gguf")
        ...
        assert artifact.download_url == "https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct-GGUF/resolve/main/qwen2.5-0.5b-instruct-q4_k_m.gguf"

    def test_resolve_explicit_filename(self) -> None:
        source = HuggingFaceModelSource()
        artifact = source.resolve("Qwen/Qwen2.5-0.5B-Instruct-GGUF", filename="model.gguf", revision="v1.0")
        assert artifact.repo_id == "Qwen/Qwen2.5-0.5B-Instruct-GGUF"
        assert artifact.filename == "model.gguf"
        assert artifact.download_url == "https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct-GGUF/resolve/v1.0/model.gguf"
...
class TestNativeModelManager:
    ...
    def test_install_success_registers_model(self, tmp_path: Path) -> None:
        ...
        download_url = "https://huggingface.co/Qwen/Qwen2.5-0.5B-GGUF/resolve/main/model.gguf"
...
class TestCLIModelAcquisition:
    ...
    def test_cli_models_pull_native(self, tmp_path: Path, monkeypatch) -> None:
        ...
        result = self.runner.invoke(
            models_cmd,
            ["pull", "test/repo/model.gguf", "--backend", "native", "--model-id", "test-model"],
        )
