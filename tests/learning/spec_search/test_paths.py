"""Tests for grandpa.learning.spec_search.storage.paths module."""
...
class TestResolveSpecSearchRoot:
    ...
    def test_default_is_under_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from grandpa.learning.spec_search.storage import paths

        monkeypatch.delenv("grandpa_HOME", raising=False)
        result = paths.resolve_spec_search_root()
        assert result == Path.home() / ".grandpa" / "learning"

    def test_respects_grandpa_home_env_var(
        self,
        tmp_path: Path,
        ...
        monkeypatch.setenv("grandpa_HOME", str(custom))
        result = paths.resolve_spec_search_root()
    ...
    def test_returns_absolute_path(
        ...
        monkeypatch.setenv("grandpa_HOME", str(tmp_path / "rel"))
        result = paths.resolve_spec_search_root()
    ...
    def test_rejects_path_inside_source_tree(
        ...
        # Find the grandpa source root by walking up from the paths module.
        source_root = paths._find_source_root()
        assert source_root is not None  # We must be running inside the repo.

        # Force grandpa_HOME to point inside the source tree.
        monkeypatch.setenv("grandpa_HOME", str(source_root / "junk_dir"))
...
class TestEnsureSpecSearchDirs:
    ...
    def test_creates_subdirs(
        ...
        monkeypatch.setenv("grandpa_HOME", str(tmp_path / "oj"))
