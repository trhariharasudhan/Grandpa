...
- TraceStore at ~/.grandpa/traces.db with some traces
"""
...
def real_trace_store():
    ...
    db_path = Path.home() / ".grandpa" / "traces.db"
    if not db_path.exists():
        pytest.skip("No traces.db found at ~/.grandpa/")
...
class TestDiagnosisRunnerLive:
    ...
    def test_diagnosis_produces_output(
        ...
        from grandpa.learning.spec_search.diagnose.runner import (
            DiagnosisRunner,
        )
...
class TestColdStartLive:
    ...
    def test_orchestrator_cold_start_with_no_feedback(
        ...
        from grandpa.learning.spec_search.gate.cold_start import (
