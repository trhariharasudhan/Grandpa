...
export function SettingsPage() {
  ...
  const [memoryEnabled, setMemoryEnabled] = useState(() => {
    try { return localStorage.getItem('Grandpa-memory-enabled') !== 'false'; } catch { return true; }
  });
  const [memoryBackend, setMemoryBackend] = useState(() => {
    try { return localStorage.getItem('Grandpa-memory-backend') || 'sqlite'; } catch { return 'sqlite'; }
  });
  const [memoryTopK, setMemoryTopK] = useState(() => {
    try { return parseInt(localStorage.getItem('Grandpa-memory-top-k') || '5'); } catch { return 5; }
  });
  const [memoryMinScore, setMemoryMinScore] = useState(() => {
    try { return parseFloat(localStorage.getItem('Grandpa-memory-min-score') || '0.1'); } catch { return 0.1; }
  });
  const [memoryMaxTokens, setMemoryMaxTokens] = useState(() => {
    try { return parseInt(localStorage.getItem('Grandpa-memory-max-tokens') || '2048'); } catch { return 2048; }
  });
  ...
  const handleExport = () => {
    const data = localStorage.getItem('Grandpa-conversations') || '{}';
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Grandpa-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.version === 1) {
            localStorage.setItem('Grandpa-conversations', JSON.stringify(data));
  ...
  const handleClear = () => {
    ...
    localStorage.removeItem('Grandpa-conversations');
    useAppStore.getState().loadConversations();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Appearance">
            <SettingRow label="Theme" description="Choose how Grandpa looks">
          ...
          <Section title="Models">
            ...
            <SettingRow label="Cloud providers" description="Green dot means API key is configured">
              <div className="flex flex-wrap gap-3">
                <CloudProviderStatus label="OpenAI" storageKey="Grandpa-openai-key" />
                <CloudProviderStatus label="Anthropic" storageKey="Grandpa-anthropic-key" />
                <CloudProviderStatus label="Google" storageKey="Grandpa-gemini-key" />
                <CloudProviderStatus label="OpenRouter" storageKey="Grandpa-openrouter-key" />
              </div>
            </SettingRow>
          </Section>
          ...
          <Section title="API Keys">
            <SettingRow label="OpenAI" description="GPT-4, GPT-3.5, etc.">
              <ApiKeyInput storageKey="Grandpa-openai-key" placeholder="sk-..." />
            </SettingRow>
            <SettingRow label="Anthropic" description="Claude models">
              <ApiKeyInput storageKey="Grandpa-anthropic-key" placeholder="sk-ant-..." />
            </SettingRow>
            <SettingRow label="Google" description="Gemini models">
              <ApiKeyInput storageKey="Grandpa-gemini-key" placeholder="AI..." />
            </SettingRow>
            <SettingRow label="OpenRouter" description="Multi-provider routing">
              <ApiKeyInput storageKey="Grandpa-openrouter-key" placeholder="sk-or-..." />
            </SettingRow>
          </Section>
          ...
          <Section title="Tools">
            <SettingRow label="Web Search" description="SerpAPI or Tavily key for web search tool">
              <ApiKeyInput storageKey="Grandpa-search-key" placeholder="API key..." />
            </SettingRow>
          </Section>
            ...
            <SettingRow label="Use memory context" description="Automatically inject relevant memories into conversations">
              <button
                onClick={() => {
                  ...
                  try { localStorage.setItem('Grandpa-memory-enabled', String(next)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Memory backend" description="Which retrieval engine to use">
              <select
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('Grandpa-memory-backend', e.target.value); } catch {}
                  showSaved();
                }}
                className="text-sm px-3 py-1.5 rounded-lg outline-none cursor-pointer"
                style={{
            ...
            <SettingRow label="Results to inject" description={`${memoryTopK}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryTopK(v);
                  try { localStorage.setItem('Grandpa-memory-top-k', String(v)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Min relevance score" description={`${memoryMinScore}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryMinScore(v);
                  try { localStorage.setItem('Grandpa-memory-min-score', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
            ...
            <SettingRow label="Max context tokens" description={`${memoryMaxTokens}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('Grandpa-memory-max-tokens', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
