/**
 * deep-link.ts — Handler for Grandpa:// deep links in the Tauri desktop app.
 *
 ...
 *   Grandpa://research/{session_id}   → open a research session
 *   Grandpa://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `Grandpa://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `Grandpa://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `Grandpa:` scheme.
 *
 * @example
 * parseDeepLink("Grandpa://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("Grandpa://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "Grandpa:") return null;

    // The URL constructor treats "Grandpa://research/abc123" such that
